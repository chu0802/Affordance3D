...
def convert_to_binary_mask(img_path, threshold=127, angle=None):
    ...
    binary_mask = (
        img_tensor > threshold
    ).float()  # Convert to binary mask, type float 32
...
def get_distance(feature1, feature2, mask1, mask2, RESOLUTION=64):
    ...
    src_feature_upsampled = F.interpolate(
        src_feature_reshaped, size=(patch_size, patch_size), mode="bilinear"
    ).squeeze()
    tgt_feature_upsampled = F.interpolate(
        tgt_feature_reshaped, size=(patch_size, patch_size), mode="bilinear"
    ).squeeze()
    # mask the feature
    src_feature_upsampled = src_feature_upsampled * resized_src_mask.repeat(
        src_feature_upsampled.shape[0], 1, 1
    )
    tgt_feature_upsampled = tgt_feature_upsampled * resized_tgt_mask.repeat(
        src_feature_upsampled.shape[0], 1, 1
    )
    ...
    src_features_2d = src_feature_upsampled.reshape(
        src_feature_upsampled.shape[0], -1
    ).permute(1, 0)
    tgt_features_2d = tgt_feature_upsampled.reshape(
        tgt_feature_upsampled.shape[0], -1
    ).permute(1, 0)
    ...
    for patch_idx in range(patch_size * patch_size):
        ...
        if resized_src_mask[patch_idx // patch_size, patch_idx % patch_size] == 1:
            ...
            distances = torch.linalg.norm(
                tgt_features_2d - src_features_2d[patch_idx], dim=1
            )
...
def pairwise_sim(
    x: torch.Tensor, y: torch.Tensor, p=2, normalize=False
) -> torch.Tensor:
...
def co_pca(features1, features2, dim=[128, 128, 128]):
    ...
    s5_2 = features2["s5"].reshape(
        features2["s5"].shape[0], features2["s5"].shape[1], -1
    )
    s4_2 = features2["s4"].reshape(
        features2["s4"].shape[0], features2["s4"].shape[1], -1
    )
    s3_2 = features2["s3"].reshape(
        features2["s3"].shape[0], features2["s3"].shape[1], -1
    )
    ...
    for name, tensors in zip(
        ["s5", "s4", "s3"], [[s5_1, s5_2], [s4_1, s4_2], [s3_1, s3_2]]
    ):
    ...
    processed_features2["s5"] = processed_features2["s5"].reshape(
        processed_features2["s5"].shape[0], -1, s5_size, s5_size
    )
    processed_features2["s4"] = processed_features2["s4"].reshape(
        processed_features2["s4"].shape[0], -1, s4_size, s4_size
    )
    processed_features2["s3"] = processed_features2["s3"].reshape(
        processed_features2["s3"].shape[0], -1, s3_size, s3_size
    )
    ...
    processed_features2["s5"] = F.interpolate(
        processed_features2["s5"],
        size=(processed_features2["s4"].shape[-2:]),
        mode="bilinear",
        align_corners=False,
    )
    ...
    processed_features2["s5"] = torch.cat(
        [processed_features2["s4"], processed_features2["s5"]], dim=1
    )
    ...
    features2_gether_s4_s5 = torch.cat(
        [
            processed_features2["s4"],
            F.interpolate(
                processed_features2["s5"],
                size=(processed_features2["s4"].shape[-2:]),
                mode="bilinear",
...
def calculate_keypoint_transformation(
    ...
    if args.SOFT_EVAL:
        ...
        nn_y_patch, nn_x_patch = flow_idxed[:, 1].clamp(0, num_patches - 1), flow_idxed[
            :, 0
        ].clamp(0, num_patches - 1)
    else:
