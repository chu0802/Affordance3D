    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        if "v2" in model_type:
            ...
            temp_state_dict = torch.load(
                "ibot/checkpoint_teacher.pth", map_location="cpu"
            )
            temp_state_dict = temp_state_dict["state_dict"]
            # remove `module.` prefix
            temp_state_dict = {
                k.replace("module.", ""): v for k, v in temp_state_dict.items()
            }
            # remove `backbone.` prefix induced by multicrop wrapper
            temp_state_dict = {
                k.replace("backbone.", ""): v for k, v in temp_state_dict.items()
            }
