...
from src.fauna.render import mesh, util
...
def laplace_regularizer_const(v_pos, t_pos_idx):
    ...
    term.scatter_add_(
        1, t_pos_idx[..., 0:1].repeat(batch_size, 1, 3), (v1 - v0) + (v2 - v0)
    )
    term.scatter_add_(
        1, t_pos_idx[..., 1:2].repeat(batch_size, 1, 3), (v0 - v1) + (v2 - v1)
    )
    term.scatter_add_(
        1, t_pos_idx[..., 2:3].repeat(batch_size, 1, 3), (v0 - v2) + (v1 - v2)
    )
