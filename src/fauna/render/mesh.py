...
from pytorch3d.structures.meshes import Meshes as Pytorch3DMeshes

from src.fauna.render import obj, util
...
class Mesh:
    ...
    def deform(self, deformation):
        ...
        return make_mesh(
            verts,
            self.t_pos_idx,
            self.v_tex.repeat(len(verts), 1, 1),
            self.t_tex_idx,
            self.material,
        )
    ...
    def to_pytorch3d(self):
        verts = self.v_pos.clone()
        faces = self.t_pos_idx.clone()
        text = TexturesAtlas([torch.ones(faces.shape[1], 1, 1, 3).to(verts.device)])
        return Pytorch3DMeshes(verts=self.v_pos, faces=faces, textures=text)
...
def make_mesh(verts, faces, uvs, uv_idx, material):
    ...
    assert (
        ...
        and len(uvs.shape) == 3
        and len(uv_idx.shape) == 3
    ), "All components must be batched."
    assert (
        faces.shape[0] == 1 and uv_idx.shape[0] == 1
    ), "Every mesh must share the same edge connectivity."
