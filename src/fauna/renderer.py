...
from src.fauna.utils import sphere
...
def get_soft_rasterizer_settings(
    image_size, sigma=1e-6, gamma=1e-6, faces_per_pixel=30
):
...
class Renderer(nn.Module):
    def __init__(self, cfgs):
        ...
        self.max_range = (
            np.tan(min(self.fov_h, self.fov_w) / 2 / 180 * np.pi)
            * self.cam_pos_z_offset
        )
        ...
        self.init_shape_scale_xy = float(cfgs.get("init_shape_scale_xy", 1.0))
        self.init_shape_scale_z = float(cfgs.get("init_shape_scale_z", 1.0))
        ...
        meshes, aux = sphere.get_symmetric_ico_sphere(
            subdiv=self.ico_sphere_subdiv,
            return_tex_uv=True,
            return_face_tex_map=True,
            device=self.device,
        )
        init_verts = meshes.verts_padded()
        self.init_verts = init_verts * torch.FloatTensor(
            [
                self.init_shape_scale_xy,
                self.init_shape_scale_xy,
                self.init_shape_scale_z,
            ]
        ).view(1, 1, 3).to(init_verts.device)
        # TODO: is this needed?
        ...
        if cfgs.get("disable_sym_tex", False):
            ...
            tex_uv_side1 = self.tex_verts_uv[
                :,
                aux["num_verts_seam"] : aux["num_verts_seam"]
                + aux["num_verts_one_side"],
            ].clone()
            ...
            tex_uv_side2 = self.tex_verts_uv[
                :, aux["num_verts_seam"] + aux["num_verts_one_side"] :
            ].clone()
            tex_uv_side2[:, :, 0] = tex_uv_side2[:, :, 0] / 2
            self.tex_verts_uv = torch.cat(
                [tex_uv_seam1, tex_uv_side1, tex_uv_side2, tex_uv_seam2], 1
            )
            ...
            face_tex_ids2[face_tex_ids2 < aux["num_verts_seam"]] += (
                aux["num_verts_seam"] + 2 * aux["num_verts_one_side"]
            )
            self.tex_faces_uv = torch.cat([face_tex_ids1, face_tex_ids2], 1)
            self.face_tex_map = torch.cat(
                [self.face_tex_map, self.face_tex_map.flip(2)], 2
            )
            self.tex_map_seam_mask = torch.cat(
                [self.tex_map_seam_mask, self.tex_map_seam_mask.flip(2)], 2
            )
    ...
    def _create_silhouette_renderer(self):
        ...
        return pytorch3d.renderer.MeshRenderer(
            rasterizer=pytorch3d.renderer.MeshRasterizer(
                cameras=self.cameras, raster_settings=settings
            ),
            shader=pytorch3d.renderer.SoftSilhouetteShader(
                cameras=self.cameras, blend_params=blend_params
            ),
        )
    ...
    def transform_verts(self, verts, pose):
        ...
        tsf = tsf.compose(
            pytorch3d.transforms.Translate(
                pose[..., -3:].view(-1, 3), device=pose.device
            )
        )
    ...
    def symmetrize_shape(self, shape):
        verts_seam = shape[:, :, : self.num_verts_seam] * torch.FloatTensor(
            [0, 1, 1]
        ).to(shape.device)
        verts_one_side = shape[
            :, :, self.num_verts_seam : self.num_verts_seam + self.num_verts_one_side
        ] * torch.FloatTensor([1, 1, 1]).to(shape.device)
        verts_other_side = verts_one_side * torch.FloatTensor([-1, 1, 1]).to(
            shape.device
        )
    ...
    def get_textures(self, tex_im):
        ...
        textures = pytorch3d.renderer.TexturesUV(
            maps=tex_im.view(b * f, *tex_im.shape[2:]).permute(
                0, 2, 3, 1
            ),  # texture maps are BxHxWx3
    ...
    def render_flow(self, meshes, shape, pose, deformed_shape=None):
        ...
        if deformed_shape is None:
            deformed_shape, meshes = self.get_deformed_mesh(
                shape.detach(), pose=pose, return_shape=True
            )
        im_size = torch.FloatTensor([self.image_size, self.image_size]).to(
            shape.device
        )  # (w,h)
        verts_2d = self.cameras.transform_points_screen(
            deformed_shape.view(b * f, *deformed_shape.shape[2:]),
            im_size.view(1, 2).repeat(b * f, 1),
            eps=1e-7,
        )
        ...
        flow_tex = torch.nn.functional.pad(
            verts_flow, pad=[0, 1, 0, 0, 0, 1]
        )  # BxFxVx3
        ...
        meshes.textures = pytorch3d.renderer.TexturesVertex(
            verts_features=flow_tex.view(b * f, -1, 3)
        )
    ...
    def forward(self, pose, texture, shape, crop_bbox=None, render_flow=True):
        ...
        deformed_shape, mesh = self.get_deformed_mesh(
            shape, pose=pose, return_shape=True
        )
        if render_flow:
            flow = self.render_flow(
                mesh, shape, pose, deformed_shape=deformed_shape
            )  # Bx(F-1)xHxWx2
