...
from src.fauna.triplane_texture.lift_architecture import Lift_Encoder
from src.fauna.triplane_texture.triplane_transformer import Triplane_Transformer
from src.fauna.utils.ddp_utils import is_main_process
...
class MLPWithPositionalEncoding(nn.Module):
    ...
    def forward(self, x, feat=None):
        assert (feat is None and self.extra_dim == 0) or feat.shape[
            -1
        ] == self.extra_dim
        if self.symmetrize:
            xs, ys, zs = x.unbind(-1)
...
class MLPTextureTriplane(nn.Module):
    def __init__(
        ...
        self.feat_net = Triplane_Transformer(
            emb_dim=256, num_layers=8, triplane_dim=80, triplane_scale=grid_scale
        )
        self.extra_dim -= extra_dim
...
class LocalFeatureBlock(nn.Module):
    def __init__(self, local_feat_dim, input_dim=384, output_dim=384, upscale_num=3):
        ...
        self.conv_head = nn.Conv2d(
            local_feat_dim, output_dim, 3, stride=1, padding=1, dilation=1
        )
...
class MLPTextureLocal(nn.Module):
    ...
    def get_uv_depth(self, xyz, mvp):
        ...
        # mvp: [b, 4, 4]
        cam4 = torch.matmul(
            torch.nn.functional.pad(xyz, pad=(0, 1), mode="constant", value=1.0),
            torch.transpose(mvp, 1, 2),
        )
        cam3 = cam4[..., :3] / cam4[..., 3:4]
        cam_uv = cam3[..., :2]
        # cam_uv = cam_uv.detach()
        cam_depth = cam3 + torch.FloatTensor([0, 0, self.cam_pos_z_offset]).to(
            xyz.device
        ).view(1, 1, 3)
    ...
    def proj_sample_deform(self, xyz, feat_map, mvp, w2c, img_h, img_w):
        ...
        if isinstance(feat_map, torch.Tensor):
            ...
            # get local feature
            feature = (
                F.grid_sample(feat_map, cam_uv.view(b, 1, k, 2), mode="bilinear")
                .squeeze(dim=-2)
                .permute(0, 2, 1)
            )  # [b, k, c]
            ...
            project_feature = (
                F.grid_sample(local_feat_map, cam_uv.view(b, 1, k, 2), mode="bilinear")
                .squeeze(dim=-2)
                .permute(0, 2, 1)
            )  # [b, k, c]
            project_depth = (
                F.grid_sample(
                    original_depth.permute(0, 3, 1, 2),
                    cam_uv.view(b, 1, k, 2),
                    mode="bilinear",
                )
                .squeeze(dim=-2)
        ...
        ret_feature = self.local_feature_block(
            feature.reshape(b * k, -1)
        )  # the linear is without bias, so 0 value feature will still get 0 value
        return ret_feature
    ...
    def proj_sample(self, xyz, feat_map, mvp, w2c, img_h, img_w, xyz_before_sym=None):
        ...
        if isinstance(feat_map, torch.Tensor):
            ...
            if xyz_before_sym is None:
                ...
                project_feature = (
                    F.grid_sample(
                        local_feat_map, cam_uv.view(b, 1, k, 2), mode="bilinear"
                    )
                    .squeeze(dim=-2)
                    .permute(0, 2, 1)
                )  # [b, k, c]
                ...
                use_mask = cam_depth <= project_depth + THRESHOLD
                feature = project_feature * use_mask.repeat(
                    1, 1, project_feature.shape[-1]
                )
            else:
                ...
                coords_rev = (
                    xyz_check  # we directly use neg-x to get the points of another side
                )
                ...
                proj_feat_inp = (
                    F.grid_sample(
                        local_feat_map, uv_inp.view(b, 1, k, 2), mode="bilinear"
                    )
                    .squeeze(dim=-2)
                    .permute(0, 2, 1)
                )  # [b, k, c]
                proj_feat_rev = (
                    F.grid_sample(
                        local_feat_map, uv_rev.view(b, 1, k, 2), mode="bilinear"
                    )
                    .squeeze(dim=-2)
                    .permute(0, 2, 1)
                )  # [b, k, c]
                ...
                feature = proj_feat_inp * use_mask_inp.repeat(
                    1, 1, proj_feat_inp.shape[-1]
                ) + proj_feat_rev * use_mask_rev.repeat(1, 1, proj_feat_rev.shape[-1])
        else:
            raise NotImplementedError
    ...
    def proj_sample_deform(self, xyz, feat_map, mvp, w2c, img_h, img_w):
        ...
        if isinstance(feat_map, torch.Tensor):
            ...
            feature = self.encoder(
                feat_map["im_features_map"], mvp, xyz, inference="sample"
            )
        C = feature.shape[-1]
...
class ResnetEncoder(nn.Module):
    def __init__(self, cout, pretrained=False):
        ...
        self.resnet = nn.Sequential(
            list(models.resnet18(weights="DEFAULT" if pretrained else None).modules())[
                :-1
            ]
        )
...
class Encoder(nn.Module):
    def __init__(self, cin, cout, in_size=128, zdim=None, nf=64, activation=None):
        ...
        network = [
            ...
            nn.Conv2d(
                nf, nf * 2, kernel_size=4, stride=2, padding=1, bias=False
            ),  # 64x64 -> 32x32
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(
                nf * 4, nf * 8, kernel_size=4, stride=2, padding=1, bias=False
            ),  # 16x16 -> 8x8
        ...
        if add_downsample > 0:
            for _ in range(add_downsample):
                network += [
                    nn.Conv2d(
                        nf * 8, nf * 8, kernel_size=4, stride=2, padding=1, bias=False
                    ),  # 16x16 -> 8x8
                    # nn.GroupNorm(16*8, nf*8),
                    # nn.ReLU(inplace=True),
                    nn.LeakyReLU(0.2, inplace=True),
        ...
        if zdim is None:
            network += [
                nn.Conv2d(
                    nf * 8, cout, kernel_size=4, stride=1, padding=0, bias=False
                ),  # 4x4 -> 1x1
            ]
        else:
            network += [
                nn.Conv2d(
                    nf * 8, zdim, kernel_size=4, stride=1, padding=0, bias=False
                ),  # 4x4 -> 1x1
                # nn.ReLU(inplace=True),
                nn.LeakyReLU(0.2, inplace=True),
...
class EncoderWithDINO(nn.Module):
    def __init__(
        self, cin_rgb, cin_dino, cout, in_size=128, zdim=None, nf=64, activation=None
    ):
        super().__init__()
        network_rgb_in = [
            nn.Conv2d(
                cin_rgb, nf, kernel_size=4, stride=2, padding=1, bias=False
            ),  # 128x128 -> 64x64
            nn.GroupNorm(16, nf),
            # nn.ReLU(inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        network_dino_in = [
            nn.Conv2d(
                cin_dino, nf, kernel_size=4, stride=2, padding=1, bias=False
            ),  # 128x128 -> 64x64
            nn.GroupNorm(16, nf),
            # nn.ReLU(inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        network_fusion = [
            nn.Conv2d(
                nf * 4 * 2, nf * 8, kernel_size=4, stride=2, padding=1, bias=False
            ),  # 16x16 -> 8x8
            # nn.GroupNorm(16*8, nf*8),
            # nn.ReLU(inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
...
class MLP(nn.Module):
    def __init__(
        ...
        if num_layers == 1:
            ...
            for _ in range(num_layers - 2):
                network += [layer_act, nn.Linear(nf, nf, bias=linear_bias)]
                if dropout:
                    network += [nn.Dropout(dropout)]
            network += [layer_act, nn.Linear(nf, cout, bias=linear_bias)]
        if activation is not None:
            network += [get_activation(activation)]
        self.network = nn.Sequential(*network)
...
class ViTEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        if is_main_process():
            force_reload = not os.path.exists(
                os.path.join(root, ".cache/torch/hub/checkpoints/")
            )
        else:
            force_reload = False
...
class ArticulationNetwork(nn.Module):
    def __init__(
        ...
        omega0=1,
        activation=None,
        enable_articulation_idadd=False,
    ):
        super().__init__()
        if n_harmonic_functions > 0:
            self.posenc = HarmonicEmbedding(
                n_harmonic_functions=n_harmonic_functions, omega0=omega0
            )
...
class FeatureAttention(nn.Module):
    def __init__(
        ...
        if embedder_freq > 0:
            self.posenc = HarmonicEmbedding(
                n_harmonic_functions=embedder_freq, omega0=1
            )
    ...
    def forward(self, x, feat):
        ...
        assert (
            ph == pw
            and ph == self.num_patches_per_dim
            and vit_feat_dim == self.vit_feat_dim
        )
