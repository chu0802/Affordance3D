...
class CUBDataset(Dataset):
    def __init__(self, opts):
        ...
        self.anno_path = osp.join(
            self.data_cache_dir, "data", "%s_cub_cleaned.mat" % self.split
        )
        self.anno_sfm_path = osp.join(
            self.data_cache_dir, "sfm", "anno_%s.mat" % self.split
        )
    ...
    def forward_img(self, index):
        ...
        bbox = (
            np.array([data.bbox.x1, data.bbox.y1, data.bbox.x2, data.bbox.y2], float)
            - 1
        )
        ...
        # Peturb bbox
        if self.split == "train":
            bbox = peturb_bbox(bbox, pf=self.padding_frac, jf=self.jitter_frac)
        else:
            bbox = peturb_bbox(bbox, pf=self.padding_frac, jf=0)
    ...
    def __getitem__(self, index):
        ...
        images = torch.FloatTensor(img / 255.0).permute(2, 0, 1).unsqueeze(0)
        ...
        bboxs = torch.FloatTensor(
            [0, 0, 0, self.img_size, self.img_size, 1, 1, 0]
        ).unsqueeze(
            0
        )  # frame_id, crop_x0, crop_y0, crop_w, crop_h, resize_sx, resize_sy, sharpness
        bg_image = images[0]
