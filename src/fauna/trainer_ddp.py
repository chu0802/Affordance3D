...
from src.fauna.dataloaders_ddp import get_sequence_loader_quadrupeds
...
class TrainerDDP:
    def __init__(self, cfgs, model):
        ...
        if len(self.train_data_dir) <= 10 and len(self.val_data_dir) <= 10:
            (
                ...
                self.val_loader,
                self.test_loader,
            ) = model.get_data_loaders_ddp(
                cfgs,
                self.dataset,
                self.rank,
                self.world_size,
                in_image_size=self.in_image_size,
                out_image_size=self.out_image_size,
            ...
            (
                ...
                self.val_loader,
                self.test_loader,
            ) = self.get_efficient_data_loaders_ddp(
                cfgs,
                self.batch_size,
                self.num_workers,
                self.in_image_size,
        ...
        if self.train_with_cub:
            ...
            (
                ...
                self.world_size,
                in_image_size=self.in_image_size,
                batch_size=self.batch_size_cub,
                num_workers=self.num_workers,
                run_train=self.run_train,
                run_test=self.run_test,
                train_data_dir=self.data_dir_cub,
                val_data_dir=self.data_dir_cub,
                test_data_dir=self.data_dir_cub,
            )
        if self.train_with_kaggle:
            ...
            (
                self.train_loader_kaggle,
                self.val_loader_kaggle,
                self.test_loader_kaggle,
            ) = model.get_data_loaders_ddp(
                cfgs,
                "kaggle",
                self.rank,
                self.world_size,
                in_image_size=self.in_image_size,
                batch_size=self.batch_size_kaggle,
                num_workers=self.num_workers,
                run_train=self.run_train,
                run_test=self.run_test,
                train_data_dir=self.data_dir_kaggle,
                val_data_dir=self.data_dir_kaggle,
                test_data_dir=self.data_dir_kaggle,
            )
    ...
    def get_efficient_data_loaders_ddp(
    ...
    def train(self):
        ...
        if misc.is_main_process() and self.use_logger:
            ...
            self.viz_data_iterator = (
                indefinite_generator_from_list(self.val_loader)
                if self.visualize_validation
                else indefinite_generator_from_list(self.train_loader)
            )
            ...
            # train with cub
            if self.train_with_cub:
                self.cub_viz_data_iterator = (
                    indefinite_generator(self.val_loader_cub)
                    if self.visualize_validation
                    else indefinite_generator(self.train_loader_cub)
                )
                if self.fix_viz_batch:
            ...
            if self.rank == 0:
                ...
                if (epoch + 1) % self.save_checkpoint_freq == 0:
                    self.save_checkpoint(
                        epoch + 1, total_iter=self.total_iter, optim=True
                    )
                if self.cfgs.get("pyplot_metrics", True):
                    self.metrics_trace.plot(
                        pdf_path=osp.join(self.checkpoint_dir, "metrics.pdf")
                    )
    ...
    def run_epoch(self, epoch):
        ...
        while iteration < max_loader_len * len(self.train_loader):
            for generator in train_generators:
                ...
                ## reset optimizers
                if self.cfgs.get(
                    "opt_reset_every_iter", 0
                ) > 0 and self.total_iter < self.cfgs.get("opt_reset_end_iter", 0):
