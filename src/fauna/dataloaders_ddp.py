...
def compute_distance_transform(mask):
    ...
    for m in mask:
        dt = torch.FloatTensor(
            cv2.distanceTransform(np.uint8(m[0]), cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
        )
        inv_dt = torch.FloatTensor(
            cv2.distanceTransform(
                np.uint8(1 - m[0]), cv2.DIST_L2, cv2.DIST_MASK_PRECISE
            )
        )
...
def crop_image(image, boxs, size):
    ...
    for box in boxs:
        ...
        crop = transforms.functional.resized_crop(
            image, crop_y0, crop_x0, crop_h, crop_w, size
        )
...
def get_valid_mask(boxs, image_size):
    ...
    for box in boxs:
        ...
        mask_full_pad = torch.nn.functional.pad(
            mask_full,
            (
                crop_w + margin_w,
                crop_w + margin_w,
        ...
        mask_full_crop = mask_full_pad[
            (crop_y0 + crop_h) : crop_y0 + (crop_h * 2),
            (crop_x0 + crop_w) : crop_x0 + (crop_w * 2),
        ]
        mask_crop = torch.nn.functional.interpolate(
            mask_full_crop[None, None, :, :], image_size, mode="nearest"
        )[0, 0]
...
class BaseSequenceDataset(Dataset):
    def __init__(
        self, root, skip_beginning=4, skip_end=4, min_seq_len=10, debug_seq=False
    ):
        super().__init__()
...
class NFrameSequenceDataset(BaseSequenceDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.image_loaders = [
            ("rgb" + rgb_suffix, torchvision.datasets.folder.default_loader)
        ]
        ...
        self.image_transform = transforms.Compose(
            [transforms.Resize(self.in_image_size), transforms.ToTensor()]
        )
        self.mask_transform = transforms.Compose(
            [
                transforms.Resize(self.out_image_size, interpolation=Image.NEAREST),
                transforms.ToTensor(),
            ]
        )
        if self.flow_loaders is not None:
            self.flow_transform = (
                lambda x: (
                    torch.FloatTensor(x.astype(np.float32)).flip(2)[:, :, :2] / 65535.0
                )
                * 2
                - 1
            )
        ...
        if load_dino_feature:
            self.dino_feature_loaders = [
                (f"feat{dino_feature_dim}.png", dino_loader, dino_feature_dim)
            ]
        self.load_dino_cluster = load_dino_cluster
        if load_dino_cluster:
            self.dino_cluster_loaders = [
                ("clusters.png", torchvision.datasets.folder.default_loader)
            ]
    ...
    def __getitem__(self, index):
        if self.random_sample:
            ...
            if len(seq) < self.num_sample_frames:
                ...
                start_frame_idx = np.random.randint(
                    len(seq) - self.num_sample_frames + 1
                )
        ...
        masks = torch.stack(
            self._load_ids(paths, self.mask_loaders, transform=self.mask_transform), 0
        )  # load all images
        ...
        if self.color_jitter is not None:
            ...
            if np.random.rand() < prob:
                jitter = True
                color_jitter_tsf_fg = transforms.ColorJitter.get_params(
                    brightness=(1 - b, 1 + b),
                    contrast=None,
                    saturation=None,
                    hue=(-h, h),
                )
                image_transform_fg = transforms.Compose(
                    [
                        transforms.Resize(self.in_image_size),
                        color_jitter_tsf_fg,
                        transforms.ToTensor(),
                    ]
                )
                color_jitter_tsf_bg = transforms.ColorJitter.get_params(
                    brightness=(1 - b, 1 + b),
                    contrast=None,
                    saturation=None,
                    hue=(-h, h),
                )
                image_transform_bg = transforms.Compose(
                    [
                        transforms.Resize(self.in_image_size),
                        color_jitter_tsf_bg,
                        transforms.ToTensor(),
        ...
        if jitter:
            images_fg = torch.stack(
                self._load_ids(paths, self.image_loaders, transform=image_transform_fg),
                0,
            )  # load all images
            images_bg = torch.stack(
                self._load_ids(paths, self.image_loaders, transform=image_transform_bg),
                0,
            )  # load all images
            ...
            images = torch.stack(
                self._load_ids(
                    paths, self.image_loaders, transform=self.image_transform
                ),
                0,
            )  # load all images
        if self.flow_bool == True and len(paths) > 1:
            flows = torch.stack(
                self._load_ids(
                    paths[:-1], self.flow_loaders, transform=self.flow_transform
                ),
                0,
            ).permute(
                0, 3, 1, 2
            )  # load flow for first image, (N-1)x(x,y)xHxW, -1~1
            flows = torch.nn.functional.interpolate(
                flows, size=self.out_image_size, mode="bilinear"
            )
        else:
            flows = torch.zeros(1)
        bboxs = torch.stack(
            self._load_ids(paths, self.bbox_loaders, transform=torch.FloatTensor), 0
        )  # load bounding boxes for all images
        mask_valid = get_valid_mask(
            bboxs, (self.out_image_size, self.out_image_size)
        )  # exclude pixels cropped outside the original image
        if self.load_background:
            bg_image = torchvision.datasets.folder.default_loader(
                os.path.join(os.path.dirname(paths[0]), "background_frame.jpg")
            )
            if jitter:
                bg_image = color_jitter_tsf_bg(bg_image)
            bg_images = crop_image(
                bg_image,
                bboxs[:, 1:5].int().numpy(),
                (self.in_image_size, self.in_image_size),
            )
        else:
        ...
        if self.load_dino_feature:
            ...
            dino_features = torch.stack(
                self._load_ids(
                    dino_paths, self.dino_feature_loaders, transform=torch.FloatTensor
                ),
                0,
            )
        ...
        if self.load_dino_cluster:
            dino_clusters = torch.stack(
                self._load_ids(
                    paths, self.dino_cluster_loaders, transform=transforms.ToTensor()
                ),
                0,
            )  # BxFx3x55x55
        else:
        ...
        if len(paths) < self.num_sample_frames:
            ...
            if dino_features.dim() > 1:
                dino_features = torch.cat(
                    [dino_features[:1]] * num_pad + [dino_features], 0
                )
            if dino_clusters.dim() > 1:
                dino_clusters = torch.cat(
                    [dino_clusters[:1]] * num_pad + [dino_clusters], 0
                )
...
class FewShotImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bboxs = torch.cat(
            [bboxs, torch.Tensor([[self.cat_num]]).float()], dim=-1
        )  # pad a label number
        ...
        if self.load_dino_feature:
            dino_features = torch.stack(
                self._load_ids(
                    paths, self.dino_feature_loaders, transform=torch.FloatTensor
                ),
                0,
            )  # BxFx64x224x224
        else:
...
class Quadrupeds_Image_Dataset(Dataset):
        ...
        **kwargs,
        ...
        for k, v in self.few_shot_data_dirs.items():
            ...
            if isinstance(v, str):
                ...
                for _v in v:
                    result = result + sorted(
                        glob(os.path.join(_v, "*" + self.image_loaders[0][0]))
                    )
            else:
            ...
            if enhance_back_view:
                ...
                back_view_result = sorted(
                    glob(os.path.join(back_view_dir, "*" + self.image_loaders[0][0]))
                )
                back_view_result = [
                    p.replace(self.image_loaders[0][0], "{}") for p in back_view_result
                ]
        ...
        self.few_shot_data_length = self._get_data_length(
            few_shot_data_paths
        )  # get the original length of each few-shot category
        ...
        if self.dataset_split_num == -1:
            self.all_data_paths, self.one_category_num = self._pad_paths(
                original_data_paths, few_shot_data_paths
            )
            ...
            (
                self.all_data_paths,
                self.one_category_num,
                self.original_category_names,
            ) = self._pad_paths_withnum(
                original_data_paths, few_shot_data_paths, self.dataset_split_num
            )
            self.all_category_num = len(self.all_data_paths.keys())
            self.all_category_names = list(self.all_data_paths.keys())
        else:
    ...
    def _pad_paths_withnum(self, ori_paths, fs_paths, split_num=1000):
        ...
        for k, v in fs_paths.items():
            if len(v) < img_num:
                ...
                new_v = new_v + v[0:pad_time]
                # new_v = mul_time * v + v[0:pad_time]
                all_paths.update({k: new_v})
            elif len(v) > img_num:
                all_paths.update({k: v[:img_num]})
            else:
                continue
    ...
    def __getitem__(self, index):
        ...
        """
        category_idx = (
            index % (self.batch_size * self.all_category_num)
        ) // self.batch_size
        path_idx = (
            (index // (self.batch_size * self.all_category_num)) * self.batch_size
            + (index % (self.batch_size * self.all_category_num))
            - category_idx * self.batch_size
        )
        ...
        flows = torch.zeros(1)
        bboxs = torch.stack(
            self._load_ids(paths, bbox_loaders, transform=torch.FloatTensor), 0
        )  # load bounding boxes for all images
        if not use_original_bbox:
            bboxs = torch.cat(
                [bboxs, torch.Tensor([[category_idx]]).float()], dim=-1
            )  # pad a label number
        ...
        if self.load_dino_feature:
            ...
            new_dino_data_path = os.path.join(
                "/viscam/projects/articulated/dor/combine_all_data_for_ablation_magicpony",
                new_dino_data_name,
            )
            ...
            if paths[0].startswith(
                ...
                new_dino_path = (
                    dino_path_list[:-2] + dino_path_list[-1:]
                )  # remove "/train/"
...
def get_sequence_loader_quadrupeds(
    original_data_dirs,
    few_shot_data_dirs,
    original_num,
    few_shot_num,
    ...
    **kwargs,
    ...
    dataset = Quadrupeds_Image_Dataset(
        original_data_dirs, few_shot_data_dirs, original_num, few_shot_num, **kwargs
    )
...
def get_sequence_loader_ddp(data_dir, world_size, rank, use_few_shot=False, **kwargs):
    ...
    if (
        isinstance(data_dir, list)
        and len(data_dir) == 2
        and isinstance(data_dir[-1], dict)
    ):
...
def get_sequence_loader_single_ddp(
    data_dir,
    world_size,
    rank,
    mode="all_frame",
...
class ImageDataset(Dataset):
    ...
    def _parse_folder(self, path):
        result = sorted(
            glob(os.path.join(path, "**/*" + self.image_loader[0]), recursive=True)
        )
    ...
    def __getitem__(self, index):
        ...
        masks = self._load_ids(
            path, self.mask_loader, transform=self.mask_transform
        ).unsqueeze(0)
        mask_dt = compute_distance_transform(masks)
        jitter = False
        if self.color_jitter is not None:
        ...
        if jitter:
            images_fg = self._load_ids(
                path, self.image_loader, transform=image_transform_fg
            ).unsqueeze(0)
            images_bg = self._load_ids(
                path, self.image_loader, transform=image_transform_bg
            ).unsqueeze(0)
            images = images_fg * masks + images_bg * (1 - masks)
        else:
            images = self._load_ids(
                path, self.image_loader, transform=self.image_transform
            ).unsqueeze(0)
        flows = torch.zeros(1)
