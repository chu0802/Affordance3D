...
class Lift_Encoder(nn.Module):
    def __init__(
        ...
        if optim_latent:
            self.optim_latent = nn.Parameter(
                torch.rand(3, feat_dim, grid_size, grid_size)
            )
        else:
    ...
    def get_coords(self, grid_size):
        with torch.no_grad():
            ...
            plane_z0 = torch.cat(
                [grids, torch.zeros(list(grids.shape[:-1]) + [1])], dim=-1
            )  # [S, S, 3]
    ...
    def get_uv_z(self, pts, mvp):
        cam4 = torch.matmul(
            torch.nn.functional.pad(pts, pad=(0, 1), mode="constant", value=1.0),
    ...
    def unproject(self, feature_map, mvp):
        ...
        feature_map = (
            self.feature_projector(feature_map.permute(0, 2, 3, 1).reshape(-1, C))
            .reshape(bs, h, w, self.nf)
            .permute(0, 3, 1, 2)
        )
        ...
        plane_feature = plane_feature.reshape(
            plane_feature.shape[0],
            3,
            self.grid_size,
            self.grid_size,
    ...
    def conv_plane(self, plane_feature):
        ...
        plane_feature = plane_feature.reshape(-1, nh, nw, nC).permute(
            0, 3, 1, 2
        )  # [bs*3, nC, nh, nw]
