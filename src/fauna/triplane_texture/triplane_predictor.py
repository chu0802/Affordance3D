...
from src.fauna.triplane_texture import misc
from src.fauna.triplane_texture.ops import (
    bias_act,
    conv2d_resample,
    fma,
    grid_sample_gradfix,
...
class SynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        self.affine = FullyConnectedLayer(
            w_dim, in_channels, bias_init=1, device=device
        )
        memory_format = (
        ...
        if use_noise:
            self.register_buffer(
                "noise_const", torch.randn([resolution, resolution], device=device)
            )
...
class ImplicitSynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.randn([out_channels, in_channels], device=device)
        )
        self.bias = torch.nn.Parameter(torch.zeros([out_channels], device=device))
    ...
    def forward(self, w, x, noise_mode="random", gain=1):
        ...
        x = bias_act.bias_act(
            ...
            clamp=act_clamp,
            dim=2,
        )  # the last dim is the feature dim
        return x
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if in_channels == 0:
            self.const = torch.nn.Parameter(
                torch.randn([out_channels, resolution, resolution], device=device)
            )
...
class ImplicitSynthesisNetwork(torch.nn.Module):
    ...
    def extra_repr(self):
        return " ".join([f"w_dim={self.w_dim:d}"])
...
class TriPlaneTex(torch.nn.Module):
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        for idx in range(n_mapping_layer):
            layer = FullyConnectedLayer(
                w_dim, w_dim, activation="lrelu", lr_multiplier=0.1, device=device
            )
        ...
        self.tri_plane_synthesis = SynthesisNetwork(
            w_dim=self.w_dim,
            img_resolution=self.tri_plane_resolution,
        ...
        self.shape_min = 0.0
        self.shape_lenght = grid_scale / 2.0
    ...
    def old_forward(self, feat, position=None, **block_kwargs):
        """
        ...
        plane_feat = self.tri_plane_synthesis(
            ws[:, : self.num_ws_tri_plane], **block_kwargs
        )
        tri_plane = torch.split(plane_feat, self.img_feat_dim, dim=1)

        normalized_tex_pos = (
            position - self.shape_min
        ) / self.shape_lenght  # in [-1, 1]
        ...
        x_feat = grid_sample_gradfix.grid_sample(
            tri_plane[0],
            torch.cat(
                [normalized_tex_pos[:, :, 0:1], normalized_tex_pos[:, :, 1:2]], dim=-1
            )
            .unsqueeze(dim=1)
            .detach(),
        )
        y_feat = grid_sample_gradfix.grid_sample(
            tri_plane[1],
            torch.cat(
                [normalized_tex_pos[:, :, 1:2], normalized_tex_pos[:, :, 2:3]], dim=-1
            )
            .unsqueeze(dim=1)
            .detach(),
        )
        z_feat = grid_sample_gradfix.grid_sample(
            tri_plane[2],
            torch.cat(
                [normalized_tex_pos[:, :, 0:1], normalized_tex_pos[:, :, 2:3]], dim=-1
            )
            .unsqueeze(dim=1)
            .detach(),
        )
    ...
    def sample(
        self, xyz, feat=None, feat_map=None, mvp=None, w2c=None, deform_xyz=None
    ):
        ...
        out = self.old_forward(feat=global_feat, position=x)
