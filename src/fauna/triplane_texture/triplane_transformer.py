...
class Transformer_layer(nn.Module):
    def __init__(self, dim_feat=384, dim=1024, hidden_dim=1024, heads=16):
        ...
        self.ffn = FeedForward(dim=dim, hidden_dim=(4 * dim), dropout=0.0)
...
class Triplane_Transformer(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            self.layers.append(
                Transformer_layer(dim_feat=384, dim=emb_dim, hidden_dim=emb_dim)
            )
    ...
    def sample_feat(self, feat_maps, pts):
        ...
        sampled_feat = torch.cat(
            [sampled_feat_xy, sampled_feat_yz, sampled_feat_xz], dim=1
        ).squeeze(
            -2
        )  # [B, F, K]
    ...
    def forward(self, feature, pts):
        ...
        triplane_feat = triplane_feat.reshape(
            triplane_feat.shape[0],
            3,
            self.triplane_dim,
            triplane_feat.shape[-2],
            triplane_feat.shape[-1],
        )
