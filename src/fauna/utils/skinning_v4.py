...
from src.fauna.utils import geometry
...
def _compute_vertices_to_bones_weights(bones_pred, seq_shape_pred, temperature=1):
    ...
    for i in range(bones_pred.shape[2]):
        vertices_to_bones += [
            geometry.line_segment_distance(
    ...
    vertices_to_bones = nn.functional.softmax(
        -torch.stack(vertices_to_bones) / temperature, dim=0
    )
...
def update_body_kinematic_chain(
    kinematic_chain,
    leg_kinematic_chain,
    body_bone_idx,
    leg_bone_idxs,
...
def lift_points_mesh(points, seq_shape, size_aspect=0.5):
    ...
    points_z_range_max = points_to_lift[..., 2] - size_aspect * (
        points_to_lift[..., 2] - points[:, :, :-2, 2]
    )
    points_z_range_min = points_to_lift[..., 2] - size_aspect * (
        points_to_lift[..., 2] - points[:, :, 2:, 2]
    )

    points_z_range_min = points_z_range_min.unsqueeze(-1).expand(
        -1, -1, -1, seq_shape.shape[-2]
    )
    points_z_range_max = points_z_range_max.unsqueeze(-1).expand(
        -1, -1, -1, seq_shape.shape[-2]
    )

    valid_points = seq_shape.unsqueeze(2).expand(
        -1, -1, points_to_lift.shape[-2], -1, -1
    )
...
def estimate_bones(
    ...
    if resample:
        ...
        seq_shape = geometry.sample_farthest_points(
    ...
    if body_bones_type == "max_distance":
        ...
        indices_max, indices_min = seq_shape[..., 2].argmax(dim=2), seq_shape[
            ..., 2
        ].argmin(dim=2)
    ...
    if n_leg_bones > 0:
        ...
        def find_leg_in_quadrant(
            quadrant, n_bones, body_bone_idx, body_bones_type=None
        ):
            all_joints = torch.zeros(
                [seq_shape.shape[0], seq_shape.shape[1], n_bones + 1, 3],
            ...
            for b in range(seq_shape.shape[0]):
                for f in range(seq_shape.shape[1]):
                    ...
                    if body_bone_idx is None:
                        body_bone_idx_1 = int(
                            torch.argmin(
                                torch.norm(bones_pred[b, f, :, 1] - foot[None], dim=1)
                            )
                        )
                        body_bone_idx_2 = int(
                            torch.argmin(
                                (bones_pred[b, f, :, 1, 2] - foot[None, 2]).abs()
                            )
                        )  # closest in z axis
                    ...
                    blend = torch.linspace(
                        0.0, 1.0, n_bones + 1, device=seq_shape.device
                    )[:, None]
        ...
        for i, quadrant in enumerate(quadrants):
            if compute_kinematic_chain:
                ...
                leg_joints, body_bone_idx = find_leg_in_quadrant(
                    quadrant,
                    n_leg_bones,
                    body_bone_idx=body_bone_idx,
                    body_bones_type=body_bones_type,
                )
                ...
                (
                    ...
                    leg_kinematic_chain,
                    leg_bone_idxs,
                ) = build_kinematic_chain(n_leg_bones, start_bone_idx=start_bone_idx)
                kinematic_chain = update_body_kinematic_chain(
                    kinematic_chain,
                    leg_kinematic_chain,
                    body_bone_idx,
                    leg_bone_idxs,
...
def skinning(
    ...
    vertices_to_bones = _compute_vertices_to_bones_weights(
        bones_pred, shape.detach(), temperature=temperature
    )  # Shape: (num_bones, B, F, V)
    ...
    for bone_id, _ in kinematic_tree:
        ...
        # Transform vertices
        shape4 = rearrange(
            torch.cat([shape, torch.ones_like(shape[..., :1])], dim=-1),
            "b f ... -> (b f) ...",
        )
        seq_shape_bone = torch.matmul(shape4, transform_mtx.transpose(-2, -1))[..., :3]
        seq_shape_bone = rearrange(
            seq_shape_bone, "(b f) ... -> b f ...", b=batch_size, f=num_frames
        )

        if output_posed_bones:
            bones4 = torch.cat(
                [
                    rearrange(posed_bones[:, :, bone_id], "b f ... -> (b f) ..."),
                    torch.ones(batch_size * num_frames, 2, 1).to(device),
                ],
                dim=-1,
            )
            posed_bones[:, :, bone_id] = rearrange(
                torch.matmul(bones4, transform_mtx.transpose(-2, -1))[..., :3],
                "(b f) ... -> b f ...",
                b=batch_size,
                f=num_frames,
            )
