...
def fig_to_img(fig, dpi=200, im_size=(512, 512)):
    ...
    buf.seek(0)
    img = np.array(Image.open(buf).convert("RGB").resize(im_size)) / 255.0
    return img
...
def get_symmetric_ico_sphere(
    subdiv=1, return_tex_uv=True, return_face_tex_map=True, device="cpu"
):
    ...
    for i, v in enumerate(rotated_verts):
        ...
        if v[0].abs() < 0.001:  # threshold 0.001
            verts_id_seam += [i]
            rotated_verts[i][0] = 0.0  # force it to be 0
    ...
    for vid in verts_id_one_side:
        verts_other_side += [
            rotated_verts[vid] * torch.FloatTensor([-1, 1, 1])
        ]  # flip x
        new_vid += 1
    ...
    for old_face in sph_faces:
        ...
        for vi in old_face:
            ...
            if vi in verts_id_seam:
                ...
                new_face1 += [vid_old_to_new[vi]]
                new_face2 += [
                    vid_old_to_new[vi] + len(verts_id_one_side)
                ]  # assuming the symmetric vertices are appended right after the original ones
            else:
                break
    ...
    if return_tex_uv:
        ...
        verts_tex_uv = verts_tex_uv / (
            (verts_tex_uv**2).sum(1, keepdim=True) ** 0.5
        ).clamp(min=1e-8)
        magnitude = (
            new_verts[:, :1].abs().acos()
        )  # set magnitude to angle deviation from vertical axis, for more even texture mapping
    ...
    if return_face_tex_map:
        ...
        face_tex_ids_one_side = face_tex_ids[
            : num_faces // 2
        ]  # assuming symmetric faces are appended right after the original ones
