...
def setup_runtime(args):
    ...
    cfgs["device"] = (
        f"cuda:{args.rank}"
        if torch.cuda.is_available() and cuda_device_id is not None
        else "cpu"
    )
...
def clean_checkpoint(checkpoint_dir, keep_num=2):
    if keep_num > 0:
        names = list(sorted(glob.glob(os.path.join(checkpoint_dir, "checkpoint*.pth"))))
...
def archive_code(arc_path, filetypes=[".py"]):
    ...
    for ftype in filetypes:
        flist.extend(
            glob.glob(
                os.path.join(cur_dir, "[!results]*", "**", "*" + ftype), recursive=True
            )
...
def save_videos(
    out_fold, imgs, prefix="", suffix="", fnames=None, ext=".mp4", cycle=False
):
    ...
    for i, fs in enumerate(imgs):
        ...
        if fnames is None:
            idx = (
                len(glob.glob(os.path.join(out_fold_i, prefix + "*" + suffix + ext)))
                + 1
            )
            fname = "%07d" % idx
        else:
        ...
        [vid.write(np.uint8(f[..., ::-1] * 255.0)) for f in fs]
        vid.release()
...
def add_text_to_image(
    img, text, pos=(12, 12), color=(1, 1, 1), font_scale=1, thickness=2
):
...
def image_grid_multi_channel(tensor, pca=False, texts=None, font_scale=0.5):
    ...
    if texts is not None:
        ...
        tensor = [
            add_text_to_image(img, text, font_scale=font_scale)
            for img, text in zip(tensor, texts)
        ]
