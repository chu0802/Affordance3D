...
from src.fauna.networks import (
    MLPWithPositionalEncoding,
    MLPWithPositionalEncoding_Style,
)
from src.fauna.render import mesh, render
...
class DMTetGeometry(torch.nn.Module):
    def __init__(
        ...
        if sdf_mode == "param":
            ...
            if dim_of_classes == 0 or (
                dim_of_classes != 0 and condition_choice == "concat"
            ):
    ...
    def get_sdf_gradient(self, class_vector=None):
        ...
        y = self.get_sdf(
            pts=sample_points, perturb_sdf=False, class_vector=class_vector
        )
    ...
    def get_sdf_reg_loss(self, class_vector=None):
        ...
        if self.sdf_mode == "mlp":
            reg_loss["sdf_gradient_reg_loss"] = (
                (self.get_sdf_gradient(class_vector=class_vector).norm(dim=-1) - 1) ** 2
            ).mean()
