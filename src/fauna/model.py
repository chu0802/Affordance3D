...
from src.fauna.geometry.dmtet import DMTetGeometry
from src.fauna.render import light, mesh, mlptexture, render, util
from src.fauna.render.regularizer import get_edge_length, normal_consistency
...
def sample_pose_hypothesis_from_quad_prediction(
    ...
    temp = 1 / np.clip(total_iter / 1000 / rot_temp_scalar, 1.0, 100.0)
...
class PriorPredictor(nn.Module):
    ...
    def forward(self, perturb_sdf=False, total_iter=None, is_training=True):
        prior_shape = self.netShape.getMesh(
            perturb_sdf=perturb_sdf, total_iter=total_iter, jitter_grid=is_training
        )
...
class InstancePredictor(nn.Module):
    ...
    def forward_articulation(
        self, shape, feat, patch_feat, mvp, w2c, batch_size, num_frames, epoch
    ):
        """
    ...
    def get_camera_extrinsics_from_pose(self, pose, znear=0.1, zfar=1000.0):
        N = len(pose)
        ...
        proj = util.perspective(self.crop_fov_approx / 180 * np.pi, 1, znear, zfar)[
            None
        ].to(pose.device)
    ...
    def forward(
        ...
        if self.enable_articulation and epoch in self.articulation_epochs:
            shape, arti_params, articulation_aux = self.forward_articulation(
                shape, feat_key, patch_key, mvp, w2c, batch_size, num_frames, epoch
            )
...
class Unsup3D:
    ...
    def forward(
        ...
        if self.enable_prior:
            ...
            prior_shape, dino_pred = self.netPrior(
                perturb_sdf=perturb_sdf, total_iter=total_iter, is_training=is_training
            )
        else:
            prior_shape = None
            raise NotImplementedError
        ...
        for name, loss in losses.items():
            ...
            if name in ["dino_feat_im_loss"]:
                loss_weight_logit = loss_weight_logit * self.cfgs.get(
        ...
        if viz_logger is not None:
            ...
            if self.render_flow and flow_gt is not None:
                ...
                large_flow_marker_mask[:, :, :, :8, :8] = 1.0
                large_flow = (
                    torch.cat([self.large_flow, self.large_flow[:, :1] * 0.0], 1)
                    .detach()
                    .cpu()[:b0]
                )
                large_flow_marker_mask = (
                    large_flow_marker_mask * large_flow[:, :, None, None, None]
                )
                red = torch.FloatTensor([1, 0, 0])[None, None, :, None, None]
                flow_gt_viz = (
                    large_flow_marker_mask * red
                    + (1 - large_flow_marker_mask) * flow_gt_viz
                )
            ...
            viz_logger.add_histogram(
                logger_prefix + "sdf",
                self.netPrior.netShape.get_sdf(perturb_sdf=False),
                total_iter,
            )
            viz_logger.add_histogram(
        ...
        if save_results:
            ...
            fnames = [
                f"{total_iter:07d}_{fid:10d}" for fid in collapseF(frame_id.int())
            ][:b0]
