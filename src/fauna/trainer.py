...
class Trainer:
    def __init__(self, cfgs, model):
        ...
        self.train_loader, self.val_loader, self.test_loader = model.get_data_loaders(
            cfgs,
            self.dataset,
    ...
    def reset_cub_viz_data_iterator(self):
        self.cub_viz_data_iterator = (
            iter(self.val_loader_cub)
            if self.visualize_validation
            else iter(self.train_loader_cub)
        )
    ...
    def train(self):
        ...
        if self.use_logger:
            ...
            self.viz_data_iterator = (
                indefinite_generator(self.val_loader)
                if self.visualize_validation
                else indefinite_generator(self.train_loader)
            )
    ...
    def run_epoch(self, epoch):
        ...
        for iteration, batch in enumerate(self.train_loader):
            ...
            if self.use_logger:
                ...
                if (
                    self.save_result_freq is not None
                ...
                if self.total_iter % self.log_freq_images == 0:
                    with torch.no_grad():
                        ...
                        if self.train_with_cub and epoch >= self.cub_start_epoch:
                            ...
                            for name, loss in m.items():
                                self.logger.add_scalar(
                                    f"cub_loss_val/{name}", loss, self.total_iter
                                )
                    torch.cuda.empty_cache()
