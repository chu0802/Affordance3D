...
from src.fauna import networks
from src.fauna.cub_dataloaders import get_cub_loader
from src.fauna.cub_dataloaders_ddp import get_cub_loader_ddp
from src.fauna.dataloaders import get_image_loader, get_sequence_loader
from src.fauna.dataloaders_ddp import get_image_loader_ddp, get_sequence_loader_ddp
from src.fauna.render.regularizer import (
    get_edge_length,
    laplace_regularizer_const,
    normal_consistency,
)
from src.fauna.renderer import *
from src.fauna.utils import misc
from src.fauna.utils.skinning_v4 import estimate_bones, skinning

from . import discriminator_architecture
from .geometry.dmtet import DMTetGeometry
from .render import light, mesh, mlptexture, render, util
...
def sample_pose_hypothesis_from_quad_prediction(
    poses_raw,
    total_iter,
    batch_size,
    num_frames,
    pose_xflip_recon=False,
    ...
    temp = 1 / np.clip(
        total_iter / 1000 / rot_temp_scalar, temp_clip_low, temp_clip_high
    )
    ...
    rots_probs = naive_probs.view(1, num_hypos) * naive_probs_weight + rots_probs * (
        1 - naive_probs_weight
    )
    ...
    if random_sample:
        ...
        best_flag = (
            torch.randperm(batch_size * num_frames, device=poses_raw.device)
            / (batch_size * num_frames)
            < np.clip((total_iter - best_pose_start_iter) / 2000, 0, 0.8)
        ).long()
    ...
    rot_pred = torch.gather(rots_pred, 1, rot_idx[:, None, None].expand(-1, 1, 3))[
        :, 0
    ]  # Nx3
...
def get_joints_20_bones(bones, aux):
    ...
    body_joints = torch.empty(
        bones.shape[0], bones.shape[1], len(body_bones_to_joints) + 1, 3
    )
    ...
    for i in range(len(leg_aux)):
        ...
        leg_joints = torch.empty(
            bones.shape[0], bones.shape[1], len(leg_aux[i]["leg_bones_to_joints"]), 3
        )
    ...
    for i in range(len(legs_aux)):
        ...
        for j in range(len(leg_bones_to_joints) - 1):
            leg_bones += [
                torch.stack(
                    [
                        joints[:, :, 9 + i * 3 + leg_bones_to_joints[j][0], :],
                        joints[:, :, 9 + i * 3 + leg_bones_to_joints[j][1], :],
                    ],
                    dim=2,
                )
            ]
        ...
        leg_bones += [
            torch.stack(
                [
                    body_bones[:, :, leg_aux["body_bone_idx"], 1, :],
                    joints[:, :, 9 + i * 3 + leg_bones_to_joints[-1][1], :],
                ],
                dim=2,
            )
        ]
    ...
    def forward(self, feat):
        ...
        if self.is_hacking:
            ...
            return torch.concat(
                [
                    self.light_dir,
                    torch.FloatTensor([self.amb, self.diff]).to(self.light_dir.device),
                ],
                -1,
            ).expand(batch_size, -1)
...
class SmoothLoss(nn.Module):
    def __init__(self, dim=0, smooth_type=None, loss_type="l2"):
        ...
        assert (
            smooth_type in supported_smooth_types
        ), f"supported smooth type: {supported_smooth_types}"
        self.smooth_type = smooth_type

        supported_loss_types = ["l2", "mse", "l1"]
        assert (
            loss_type in supported_loss_types
        ), f"supported loss type: {supported_loss_types}"
    ...
    def mid_frame_smooth(self, inputs):
        ...
        mid_frame = torch.index_select(
            inputs, self.dim, torch.tensor([mid_num], device=inputs.device)
        )
    ...
    def dislocation_smooth(self, inputs):
        ...
        t = torch.index_select(
            inputs, self.dim, torch.arange(0, nframe - 1).to(inputs.device)
        )
        t_1 = torch.index_select(
            inputs, self.dim, torch.arange(1, nframe).to(inputs.device)
        )
...
class PriorPredictor(nn.Module):
    def __init__(self, cfgs):
        ...
        jitter_grid = cfgs.get("jitter_grid", 0.0)
        ...
        self.netShape = DMTetGeometry(
            ...
            perturb_sdf_iter=perturb_sdf_iter,
            sym_prior_shape=sym_prior_shape,
            dim_of_classes=dim_of_classes,
        ...
        dino_min = torch.zeros(dino_feature_recon_dim) + cfgs.get("dino_min", 0.0)
        dino_max = torch.zeros(dino_feature_recon_dim) + cfgs.get("dino_max", 1.0)
        ...
        if self.render_dino_mode is None:
            ...
            self.netDINO = mlptexture.MLPTexture3D(
                tet_bbox,
                channels=dino_feature_recon_dim,
                internal_dims=mlp_hidden_size,
                hidden=num_layers_dino - 1,
                feat_dim=0,
                min_max=min_max,
                bsdf=None,
        ...
        if num_of_classes > 1:
            self.classes_vectors = torch.nn.Parameter(
                torch.nn.init.uniform_(
                    torch.empty(num_of_classes, dim_of_classes), a=-0.05, b=0.05
                )
            )
    ...
    def forward(
        ...
        prior_shape = self.netShape.getMesh(
            perturb_sdf=perturb_sdf,
            total_iter=total_iter,
            jitter_grid=is_training,
...
class InstancePredictor(nn.Module):
    def __init__(self, cfgs, tet_bbox=None):
        ...
        if self.enable_encoder:
            ...
            if encoder_arch == "simple":
                if self.dino_feature_input:
                    self.netEncoder = networks.EncoderWithDINO(
                        cin_rgb=3,
                        cin_dino=dino_feature_dim,
                ...
                self.netEncoder = networks.VGGEncoder(
                    cout=encoder_latent_dim, pretrained=encoder_pretrained
                )
            elif encoder_arch == "resnet":
                self.netEncoder = networks.ResnetEncoder(
                    cout=encoder_latent_dim, pretrained=encoder_pretrained
                )
        ...
        kd_min = torch.FloatTensor(cfgs.get("kd_min", [0.0, 0.0, 0.0, 0.0]))
        kd_max = torch.FloatTensor(cfgs.get("kd_max", [1.0, 1.0, 1.0, 1.0]))
        ks_min = torch.FloatTensor(cfgs.get("ks_min", [0.0, 0.0, 0.0]))
        ks_max = torch.FloatTensor(cfgs.get("ks_max", [0.0, 0.0, 0.0]))
        nrm_min = torch.FloatTensor(cfgs.get("nrm_min", [-1.0, -1.0, 0.0]))
        nrm_max = torch.FloatTensor(cfgs.get("nrm_max", [1.0, 1.0, 1.0]))
        ...
        if texture_mode == "mlpnv":
            self.netTexture = mlptexture.MLPTexture3D(
                tet_bbox,
                channels=out_chn,
                internal_dims=mlp_hidden_size,
                hidden=num_layers_tex - 1,
                feat_dim=feat_dim,
                min_max=min_max,
                bsdf=bsdf,
            ...
            if self.texture_way is None:
                ...
                self.netTexture = networks.MLPTextureTriplane(
                    ...
                    texture_act="relu",
                    linear_bias=False,
                    cam_pos_z_offset=cfgs.get("cam_pos_z_offset", 10.0),
        ...
        if self.enable_pose:
            ...
            self.max_trans_xy_range = half_range * cfgs.get(
                "max_trans_xy_range_ratio", 1.0
            )
            self.max_trans_z_range = half_range * cfgs.get(
                "max_trans_z_range_ratio", 1.0
            )
            self.lookat_init = cfgs.get("lookat_init", None)
            self.lookat_zeroy = cfgs.get("lookat_zeroy", False)
            self.rot_temp_scalar = cfgs.get("rot_temp_scalar", 1.0)
            ...
            if self.rot_rep == "euler_angle":
                ...
                pose_cout = (
                    3 + 1
                ) * self.num_pose_hypos + 3  # 4 forward vectors for 4 quadrants, 4 quadrant classification logits, 3 for translation
                self.orthant_signs = torch.FloatTensor(
                ...
                pose_cout = (
                    3 + 1
                ) * self.num_pose_hypos + 3  # 4 forward vectors for 8 octants, 8 octant classification logits, 3 for translation
                self.orthant_signs = torch.stack(
                    torch.meshgrid([torch.arange(1, -2, -2)] * 3), -1
                ).view(
                    -1, 3
                )  # 8x3
            else:
            ...
            if self.pose_arch == "mlp":
                ...
                if self.dino_feature_input:
                    dino_feature_dim = cfgs.get("dino_feature_dim", 64)
                    self.netPose = networks.EncoderWithDINO(
                        cin_rgb=3,
                        cin_dino=dino_feature_dim,
                        cout=pose_cout,
                        in_size=in_image_size,
                ...
                vit_final_layer_type = cfgs.get("vit_final_layer_type", "conv")
                root_dir = cfgs.get("root_dir", "/root")
                self.netPose = networks.ViTEncoder(
                    cout=encoder_latent_dim,
                    which_vit=which_vit,
                    pretrained=encoder_pretrained,
                    frozen=encoder_frozen,
        ...
        if self.enable_articulation:
            ...
            self.articulation_epochs = np.arange(
                *cfgs.get("articulation_epochs", [0, 0])
            )
            ...
            self.perturb_articulation_epochs = np.arange(
                *cfgs.get("perturb_articulation_epochs", [0, 0])
            )
            self.num_bones = self.num_body_bones + self.num_legs * self.num_leg_bones
            self.constrain_legs = cfgs.get("constrain_legs", False)
            self.attach_legs_to_body_epochs = np.arange(
                *cfgs.get("attach_legs_to_body_epochs", [0, 0])
            )
            ...
            self.articulation_feature_mode = cfgs.get(
                "articulation_feature_mode", "sample"
            )
            ...
            if self.articulation_feature_mode == "attention":
                ...
                pos_dim = 1 + 2 + 3 * 2
                self.netFeatureAttn = networks.FeatureAttention(
                    which_vit,
                    pos_dim,
                    embedder_freq_arti,
                    arti_feat_attn_zdim,
                    img_size=in_image_size,
                )
            embedder_scaler = (
                np.pi * 0.9
            )  # originally (-1, 1) rescale to (-pi, pi) * 0.9
            enable_articulation_idadd = cfgs.get("enable_articulation_idadd", False)
            self.netArticulation = networks.ArticulationNetwork(
                self.articulation_arch,
                feat_dim,
                1 + 2 + 3 * 2,
                num_layers_arti,
                mlp_hidden_size,
        ...
        if self.enable_lighting:
            ...
            amb_diff_min = torch.FloatTensor(cfgs.get("amb_diff_min", [0.0, 0.0]))
            amb_diff_max = torch.FloatTensor(cfgs.get("amb_diff_max", [1.0, 1.0]))
            intensity_min_max = torch.stack((amb_diff_min, amb_diff_max), dim=0)
            self.netLight = light.DirectionalLight(
                encoder_latent_dim,
                num_layers_light,
                mlp_hidden_size,
                intensity_min_max=intensity_min_max,
            )

        self.cam_pos_z_offset = cfgs.get("cam_pos_z_offset", 10.0)
        self.crop_fov_approx = cfgs.get("crop_fov_approx", 25)

        self.temp_clip_low = cfgs.get("temp_clip_low", 1.0)
        self.temp_clip_high = cfgs.get("temp_clip_high", 100.0)
        ...
        self.iter_attach_leg_to_body_start = cfgs.get(
            "iter_attach_leg_to_body_start", None
        )
    ...
    def forward_encoder(self, images, dino_features=None):
        ...
        if self.dino_feature_input and self.cfgs.get("encoder_arch", "simple") != "vit":
            dino_features_in = (
                dino_features.view(-1, *dino_features.shape[2:]) * 2 - 1
            )  # rescale to (-1, 1)
            feat_out = self.netEncoder(
                images_in, dino_features_in
            )  # Shape: (B, latent_dim)
        elif self.cfgs.get("encoder_arch", "simple") == "vit":
            feat_out, feat_key, patch_out, patch_key = self.netEncoder(
    ...
    def forward_pose(self, images, feat, patch_out, patch_key, dino_features):
        if self.pose_arch == "mlp":
            ...
            raise NotImplementedError
        trans_pred = pose[..., -3:].tanh() * torch.FloatTensor(
            [self.max_trans_xy_range, self.max_trans_xy_range, self.max_trans_z_range]
        ).to(pose.device)
        if self.rot_rep == "euler_angle":
            multiplier = 1.0
            rot_pred = torch.cat(
                [pose[..., :1], pose[..., 1:2] * multiplier, pose[..., 2:3]], -1
            ).tanh()
            rot_pred = (
                rot_pred
                * torch.FloatTensor(
                    [self.max_rot_x_range, self.max_rot_y_range, self.max_rot_z_range]
                ).to(pose.device)
                / 180
                * np.pi
            )
            ...
            rot_pred = (
                rot_pred * rot_pred[..., :1].sign()
            )  # make real part non-negative

        elif self.rot_rep == "lookat":
            vec_forward_raw = pose[..., :3]
            if self.lookat_init is not None:
                vec_forward_raw = vec_forward_raw + torch.FloatTensor(
                    self.lookat_init
                ).to(pose.device)
            if self.lookat_zeroy:
                vec_forward_raw = vec_forward_raw * torch.FloatTensor([1, 0, 1]).to(
                    pose.device
                )
            vec_forward_raw = nn.functional.normalize(
                vec_forward_raw, p=2, dim=-1
            )  # x right, y up, z forward
            rot_pred = vec_forward_raw

        elif self.rot_rep in ["quadlookat", "octlookat"]:
            rots_pred = pose[..., : self.num_pose_hypos * 4].view(
                -1, self.num_pose_hypos, 4
            )  # (B, T, K, 4)
            ...
            margin = 0.0
            xs = (
                nn.functional.softplus(xs, beta=np.log(2) / (0.5 + margin)) - margin
            )  # initialize to 0.5
            if self.rot_rep == "octlookat":
                ys = (
                    nn.functional.softplus(ys, beta=np.log(2) / (0.5 + margin)) - margin
                )  # initialize to 0.5
            if self.lookat_zeroy:
                ys = ys * 0
            ...
            rot_pred = torch.cat([rots_logits, vec_forward_raw], -1).view(
                -1, self.num_pose_hypos * 4
            )
    ...
    def forward_deformation(self, shape, feat=None, batch_size=None, num_frames=None):
        ...
        if feat is not None:
            deform_feat = feat[:, None, :].repeat(
                1, num_verts, 1
            )  # Shape: (B, num_verts, latent_dim)
            original_verts = original_verts.repeat(len(feat), 1, 1)
        deformation = (
            self.netDeform(original_verts, deform_feat) * 0.1
        )  # Shape: (B, num_verts, 3)
        # if self.avg_deform:
    ...
    def forward_articulation(
        ...
        if self.kinematic_tree_epoch != epoch:
            ...
            if (
                total_iter is not None
                and self.iter_attach_leg_to_body_start is not None
            ):
            ...
            # trivial set here
            body_bone_idx_preset_cfg = self.cfgs.get(
            ...
            if isinstance(body_bone_idx_preset_cfg, list):
                ...
                if total_iter <= iter_point:
                    body_bone_idx_preset = body_bone_idx_preset_cfg[
                        0
                    ]  # the first is start from 0 iter
                else:
            ...
            bones, self.kinematic_tree, self.bone_aux = estimate_bones(
                ...
                bone_y_threshold=bone_y_thresh,
                body_bone_idx_preset=body_bone_idx_preset,
            )
        ...
        if batch_size > bones_pos.shape[0] or num_frames > bones_pos.shape[1]:
            assert (
                bones_pos.shape[0] == 1 and bones_pos.shape[1] == 1
            ), "If there is a mismatch, then there must be only one canonical mesh."
        ...
        bones_mid_pos_world4 = torch.cat(
            [bones_mid_pos, torch.ones_like(bones_mid_pos[..., :1])], -1
        )  # NxKx4
        ...
        bones_pos_world4 = torch.cat(
            [bones_pos, torch.ones_like(bones_pos[..., :1])], -1
        )  # NxKx2x4
        ...
        bones_pos_cam3 = bones_pos_cam3 + torch.FloatTensor(
            [0, 0, self.cam_pos_z_offset]
        ).to(bones_pos_cam3.device).view(1, 1, 1, 3)
        bones_pos_in = (
            bones_pos_cam3.view(batch_size * num_frames, num_bones, 2 * 3)
            / self.grid_scale
            * 2
        )  # (-1, 1), NxKx(2*3)

        bones_idx_in = ((bones_idx[None, :, None] + 0.5) / num_bones * 2 - 1).repeat(
            batch_size * num_frames, 1, 1
        )  # (-1, 1)
        bones_pos_in = torch.cat(
            [bones_mid_pos_uv, bones_pos_in, bones_idx_in], -1
        ).detach()

        if self.articulation_feature_mode == "global":
            bones_patch_features = feat[:, None].repeat(
                1, num_bones, 1
            )  # (BxF, K, feat_dim)
        elif self.articulation_feature_mode == "sample":
            bones_patch_features = (
                F.grid_sample(
                    patch_feat,
                    bones_mid_pos_uv.view(batch_size * num_frames, 1, -1, 2),
                    mode="bilinear",
                )
                .squeeze(dim=-2)
                .permute(0, 2, 1)
            ...
            bones_patch_features = torch.cat(
                [feat[:, None].repeat(1, num_bones, 1), bones_patch_features], -1
            )
        ...
        articulation_angles = (
            self.netArticulation(bones_patch_features, bones_pos_in).view(
                batch_size, num_frames, num_bones, 3
            )
            * self.articulation_multiplier
        )
        ...
        if (self.cfgs.get("iter_leg_rotation_start", -1) > 0) and (
            total_iter > self.cfgs.get("iter_leg_rotation_start", -1)
        ):
        ...
        if epoch in self.perturb_articulation_epochs:
            articulation_angles = (
                articulation_angles + torch.randn_like(articulation_angles) * 0.1
            )
        ...
        body_rotate_mult = (
            body_rotate_mult * 180 * 1.0 / (self.max_arti_angle * np.pi)
        ...
        if body_rotate_reg_mode == "leg-connect":
            ...
            articulation_angles = articulation_angles * 1.0
        ...
        verts_articulated, aux = skinning(
            ...
            temperature=self.skinning_temperature,
        )
        verts_articulated = verts_articulated.view(
    ...
    def get_camera_extrinsics_from_pose(
        self, pose, znear=0.1, zfar=1000.0, crop_fov_approx=None, offset_extra=None
    ):
        if crop_fov_approx is None:
            crop_fov_approx = self.crop_fov_approx
        N = len(pose)
        if offset_extra is not None:
            cam_pos_offset = torch.FloatTensor(
                [0, 0, -self.cam_pos_z_offset - offset_extra]
            ).to(pose.device)
        else:
            cam_pos_offset = torch.FloatTensor([0, 0, -self.cam_pos_z_offset]).to(
                pose.device
            )
        ...
        if isinstance(crop_fov_approx, float) or isinstance(crop_fov_approx, int):
            proj = util.perspective(crop_fov_approx / 180 * np.pi, 1, znear, zfar)[
                None
            ].to(pose.device)
    ...
    def forward(
        ...
        if self.enable_encoder:
            feat_out, feat_key, patch_out, patch_key = self.forward_encoder(
        ...
        if (
            self.iter_nozeroy_start is not None
            and total_iter >= self.iter_nozeroy_start
        ):
            self.lookat_zeroy = False

        if self.enable_pose:
            poses_raw = self.forward_pose(
                images, feat_out, patch_out, patch_key, dino_features
            )
            (
                pose_raw,
                pose,
                ...
                rots_probs,
                rand_pose_flag,
            ) = sample_pose_hypothesis_from_quad_prediction(
                poses_raw,
                total_iter,
                batch_size,
                num_frames,
                rot_temp_scalar=self.rot_temp_scalar,
                num_hypos=self.num_pose_hypos,
                naive_probs_iter=self.naive_probs_iter,
                best_pose_start_iter=self.best_pose_start_iter,
                random_sample=is_training,
                temp_clip_low=self.temp_clip_low,
                temp_clip_high=self.temp_clip_high,
            )
        ...
        if self.iter_deformation_start is not None:
            if self.enable_deform and total_iter >= self.iter_deformation_start:
                shape, deformation = self.forward_deformation(
                    shape, feat_key, batch_size, num_frames
                )
        else:
...
class Unsup3DDDP:
    def __init__(self, cfgs):
        ...
        if self.use_scheduler:
            ...
            self.make_scheduler = lambda optim: torch.optim.lr_scheduler.MultiStepLR(
                optim, milestones=scheduler_milestone, gamma=scheduler_gamma
            )
        ...
        if self.enable_prior:
            ...
            self.prior_weight_decay = cfgs.get("prior_weight_decay", 0.0)
            self.prior_only_epochs = cfgs.get("prior_only_epochs", 0)
        self.netInstance = InstancePredictor(
            self.cfgs, tet_bbox=self.netPrior.netShape.getAABB()
        )
        ...
        self.seqshape_epochs = np.arange(
            *cfgs.get("seqshape_epochs", [0, self.num_epochs])
        )
        ...
        self.swap_priorshape_epochs = np.arange(
            *cfgs.get("swap_priorshape_epochs", [0, 0])
        )
        ...
        self.texture_epochs = np.arange(
            *cfgs.get("texture_epochs", [0, self.num_epochs])
        )
        self.zflip_epochs = np.arange(*cfgs.get("zflip_epochs", [0, self.num_epochs]))
        self.lookat_zflip_loss_epochs = np.arange(
            *cfgs.get("lookat_zflip_loss_epochs", [0, self.num_epochs])
        )
        self.lookat_zflip_no_other_losses = cfgs.get(
            "lookat_zflip_no_other_losses", False
        )
        self.flow_loss_epochs = np.arange(
            *cfgs.get("flow_loss_epochs", [0, self.num_epochs])
        )
        self.sdf_inflate_reg_loss_epochs = np.arange(
            *cfgs.get("sdf_inflate_reg_loss_epochs", [0, self.num_epochs])
        )
        self.arti_reg_loss_epochs = np.arange(
            *cfgs.get("arti_reg_loss_epochs", [0, self.num_epochs])
        )
        ...
        self.pose_xflip_recon_epochs = np.arange(
            *cfgs.get("pose_xflip_recon_epochs", [0, 0])
        )
        ...
        self.arti_smooth_loss_weight = cfgs.get("arti_smooth_loss_weight", 0.0)
        self.using_arti_smooth_loss = (
            self.arti_smooth_type
            and self.arti_smooth_loss_type
            and self.arti_smooth_loss_weight > 0.0
        )
        if self.using_arti_smooth_loss:
            self.arti_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.arti_smooth_type,
                loss_type=self.arti_smooth_loss_type,
            )
        else:
        ...
        self.deform_smooth_loss_weight = cfgs.get("deform_smooth_loss_weight", 0.0)
        self.using_deform_smooth_loss = (
            self.deform_smooth_type
            and self.deform_smooth_loss_type
            and self.deform_smooth_loss_weight > 0.0
        )
        if self.using_deform_smooth_loss:
            self.deform_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.deform_smooth_type,
                loss_type=self.deform_smooth_loss_type,
            )
        else:
        ...
        self.campos_smooth_loss_weight = cfgs.get("campos_smooth_loss_weight", 0.0)
        self.using_campos_smooth_loss = (
            self.campos_smooth_type
            and self.campos_smooth_loss_type
            and self.campos_smooth_loss_weight > 0.0
        )
        if self.using_campos_smooth_loss:
            self.campos_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.campos_smooth_type,
                loss_type=self.campos_smooth_loss_type,
            )
        else:
        ...
        self.artivel_smooth_loss_weight = cfgs.get("artivel_smooth_loss_weight", 0.0)
        self.using_artivel_smooth_loss = (
            self.artivel_smooth_type
            and self.artivel_smooth_loss_type
            and self.artivel_smooth_loss_weight > 0.0
        )
        if self.using_artivel_smooth_loss:
            self.artivel_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.artivel_smooth_type,
                loss_type=self.artivel_smooth_loss_type,
            )
        else:
        ...
        self.bone_smooth_loss_weight = cfgs.get("bone_smooth_loss_weight", 0.0)
        self.using_bone_smooth_loss = (
            self.bone_smooth_type
            and self.bone_smooth_loss_type
            and self.bone_smooth_loss_weight > 0.0
        )
        if self.using_bone_smooth_loss:
            self.bone_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.bone_smooth_type,
                loss_type=self.bone_smooth_loss_type,
            )
        else:
        ...
        self.bonevel_smooth_loss_weight = cfgs.get("bonevel_smooth_loss_weight", 0.0)
        self.using_bonevel_smooth_loss = (
            self.bonevel_smooth_type
            and self.bonevel_smooth_loss_type
            and self.bonevel_smooth_loss_weight > 0.0
        )
        if self.using_bonevel_smooth_loss:
            self.bonevel_smooth_loss_fn = SmoothLoss(
                dim=1,
                smooth_type=self.bonevel_smooth_type,
                loss_type=self.bonevel_smooth_loss_type,
            )
        else:
            self.bonevel_smooth_loss_fn = None

        ## perceptual loss
        if cfgs.get("perceptual_loss_weight", 0.0) > 0:
            self.perceptual_loss_use_lin = cfgs.get("perceptual_loss_use_lin", True)
            self.perceptual_loss = lpips.LPIPS(
                net="vgg", lpips=self.perceptual_loss_use_lin
            )

        # self.glctx = dr.RasterizeGLContext()
        self.glctx = dr.RasterizeCudaContext()
        self.render_flow = self.cfgs.get("flow_loss_weight", 0.0) > 0.0
        ...
        self.random_mask_law = cfgs.get(
            "random_mask_law", "batch_swap_noy"
        )  # batch_swap, batch_swap_noy, # random_azimuth # random_all
    ...
    def get_data_loaders(
        ...
        if dataset == "video":
            ...
            if run_train:
                assert osp.isdir(
                    train_data_dir
                ), f"Training data directory does not exist: {train_data_dir}"
                print(f"Loading training data from {train_data_dir}")
                train_loader = get_loader(
                    data_dir=train_data_dir,
                    is_validation=False,
                    random_sample=random_sample_train_frames,
                ...
                if val_data_dir is not None:
                    assert osp.isdir(
                        val_data_dir
                    ), f"Validation data directory does not exist: {val_data_dir}"
                    print(f"Loading validation data from {val_data_dir}")
                    val_loader = get_loader(
                        data_dir=val_data_dir,
                        is_validation=True,
                        random_sample=random_sample_val_frames,
                        shuffle=False,
                        dense_sample=False,
            ...
            if run_test:
                assert osp.isdir(
                    test_data_dir
                ), f"Testing data directory does not exist: {test_data_dir}"
        ...
        ## CUB dataset
            ...
            if run_train:
                ...
                print(f"Loading training data from {train_data_dir}")
                train_loader = get_loader(
                    data_dir=train_data_dir, split="train", is_validation=False
                )
                val_loader = get_loader(
                    data_dir=val_data_dir, split="val", is_validation=True
                )
            ...
            if run_test:
                ...
                print(f"Loading testing data from {test_data_dir}")
                test_loader = get_loader(
                    data_dir=test_data_dir, split="test", is_validation=True
                )
        ...
        ## other datasets
            ...
            if run_test:
                ...
                print(f"Loading testing data from {test_data_dir}")
                test_loader = get_loader(
                    data_dir=test_data_dir, is_validation=True, color_jitter=None
                )

        return train_loader, val_loader, test_loader

    @staticmethod
    def get_data_loaders_ddp(
        cfgs,
        dataset,
        rank,
        world_size,
            ...
            if run_train:
                if isinstance(train_data_dir, dict):
                    for data_path in train_data_dir.values():
                        assert osp.isdir(
                            data_path
                        ), f"Training data directory does not exist: {data_path}"
                    ...
                    raise ValueError(
                        "train_data_dir must be a string or a dict of strings"
                    )
                ...
                if val_data_dir is not None:
                    if isinstance(val_data_dir, dict):
                        ...
                        assert osp.isdir(
                            val_data_dir
                        ), f"Training data directory does not exist: {val_data_dir}"
                    else:
                        raise ValueError(
                            "train_data_dir must be a string or a dict of strings"
                        )
    ...
    def load_model_state(self, cp):
        ...
        if (self.netInstance.texture_way is not None) or (
            self.cfgs.get("texture_act", "relu") != "relu"
        ):
            new_netInstance_weights = {
                k: v for k, v in cp["netInstance"].items() if "netTexture" not in k
            }
    ...
    def load_optimizer_state(self, cp):
        ...
        if self.netInstance.texture_way is not None:
            ...
            new_opt_state_dict = {}
            new_opt_state_dict["state"] = {
                k: v for k, v in opt_state_dict["state"].items() if k not in param_ids
            }
            ...
            for param_group in opt_state_dict["param_groups"]:
                ...
                new_param_group["params"] = [
                    p_id for p_id in param_group["params"] if p_id not in param_ids
                ]
    ...
    def ddp(self, rank, world_size):
        ...
        if self.world_size > 1:
            self.netInstance_ddp = DDP(
                self.netInstance, device_ids=[rank], find_unused_parameters=True
            )
            ...
            if self.enable_prior:
                self.netPrior_ddp = DDP(
                    self.netPrior, device_ids=[rank], find_unused_parameters=True
                )
            ...
            if hasattr(self, "perceptual_loss"):
                self.perceptual_loss_ddp = DDP(
                    self.perceptual_loss, device_ids=[rank], find_unused_parameters=True
                )
    ...
    def reset_optimizers(self):
        ...
        if self.enable_prior:
            self.optimizerPrior = get_optimizer(
                self.netPrior, lr=self.prior_lr, weight_decay=self.prior_weight_decay
            )
            if self.use_scheduler:
    ...
    def zflip_pose(self, pose):
        if self.rot_rep == "lookat":
            ...
            vec_forward = vec_forward * torch.FloatTensor([1, 1, -1]).view(1, 1, 3).to(
                vec_forward.device
            )
        ...
        if bg_image is None:
            if background in ["none", "black"]:
                ...
                bg_image = torch.FloatTensor(
                    util.checkerboard((h, w), 8), device=self.device
                ).repeat(
                    N, 1, 1, 1
                )  # NxHxWxC
    ...
    def compute_reconstruction_losses(
        ...
        losses["silhouette_dt_loss"] = (
            (mask_pred * mask_dt[:, :, 1]).view(batch_size, num_frames, -1).mean(2)
        )
        losses["silhouette_inv_dt_loss"] = (
            ((1 - mask_pred) * mask_dt[:, :, 0])
            .view(batch_size, num_frames, -1)
            .mean(2)
        )

        mask_pred_binary = (mask_pred_valid > 0.0).float().detach()
        mask_both_binary = (mask_pred_binary * mask_gt).view(
            batch_size * num_frames, 1, *mask_pred.shape[2:]
        )
        mask_both_binary = (
            (
                nn.functional.avg_pool2d(mask_both_binary, 3, stride=1, padding=1).view(
                    batch_size, num_frames, *mask_pred.shape[2:]
                )
                > 0.99
            )
            .float()
        ...
        if self.cfgs.get("perceptual_loss_weight", 0.0) > 0:
            if background_mode in ["background", "input"]:
                perc_image_pred = image_pred
                perc_image_gt = image_gt
            else:
                perc_image_pred = image_pred * mask_pred_binary.unsqueeze(2) + 0.5 * (
                    1 - mask_pred_binary.unsqueeze(2)
                )
                perc_image_gt = image_gt * mask_pred_binary.unsqueeze(2) + 0.5 * (
                    1 - mask_pred_binary.unsqueeze(2)
                )
            losses["perceptual_loss"] = self.perceptual_loss(
                perc_image_pred.view(-1, *image_pred.shape[2:]) * 2 - 1,
                perc_image_gt.view(-1, *image_gt.shape[2:]) * 2 - 1,
        ...
        if flow_pred is not None:
            ...
            flow_loss_mask = (
                mask_both_binary[:, :-1].unsqueeze(2).expand_as(flow_gt).detach()
            )

            ## ignore frames where GT flow is too large (likely inaccurate)
            large_flow = (flow_gt.abs() > 0.5).float() * flow_loss_mask
            large_flow = (
                large_flow.view(batch_size, num_frames - 1, -1).sum(2) > 0
            ).float()
            self.large_flow = large_flow

            flow_loss = (
                flow_loss * flow_loss_mask * (1 - large_flow[:, :, None, None, None])
            )
            num_mask_pixels = (
                flow_loss_mask.reshape(batch_size, num_frames - 1, -1)
                .sum(2)
                .clamp(min=1)
            )
            losses["flow_loss"] = (
                flow_loss.reshape(batch_size, num_frames - 1, -1).sum(2)
                / num_mask_pixels
            )
        ...
        if dino_feat_im_pred is not None and dino_feat_im_gt is not None:
            ...
            losses["dino_feat_im_loss"] = dino_feat_loss.reshape(
    ...
    def compute_regularizers(
        ...
        if self.enable_prior:
            losses.update(
                self.netPrior.netShape.get_sdf_reg_loss(class_vector=class_vector)
            )
        ...
        if b >= 2:
            ...
            losses["pose_entropy_loss"] = (
                (vec_forward[: b // 2] * vec_forward[b // 2 : (b // 2) * 2])
                .sum(-1)
                .mean()
            )
        else:
            losses["pose_entropy_loss"] = 0.0

        losses["mesh_normal_consistency_loss"] = normal_consistency(
            mesh.v_pos, mesh.t_pos_idx
        )
        losses["mesh_laplacian_consistency_loss"] = laplace_regularizer_const(
            mesh.v_pos, mesh.t_pos_idx
        )
        (
            losses["mesh_edge_length_loss"],
            aux["edge_lengths"],
        ) = self.compute_edge_length_reg_loss(mesh, prior_mesh)
    ...
    def compute_clip_loss(self, random_image_pred, image_pred, category):
        ...
        random_image = torch.nn.functional.interpolate(
            random_image_pred, (self.clip_reso, self.clip_reso), mode="bilinear"
        )
        image_pred = torch.nn.functional.interpolate(
            image_pred.squeeze(1), (self.clip_reso, self.clip_reso), mode="bilinear"
        )
        ...
        if self.enable_clip_text:
            text_feature = self.clip_text_feature[category].repeat(
                feat_img_1.shape[0], 1
            )

            text_loss_1 = torch.nn.functional.cosine_similarity(
                feat_img_1, text_feature
            ).mean()
            text_loss_2 = torch.nn.functional.cosine_similarity(
                feat_img_2, text_feature
            ).mean()
    ...
    def generate_patch_crop(self, images, masks, patch_size=128, patch_num_per_mask=1):
        ...
        for i in range(masks.shape[0]):
            ...
            valid_mask = (
                (nonzero_indices[:, 1] > patch_size // 2)
                & (nonzero_indices[:, 1] < (H - 1 - patch_size // 2))
                & (nonzero_indices[:, 2] > patch_size // 2)
                & (nonzero_indices[:, 2] < (W - 1 - patch_size // 2))
            )
            valid_idx = nonzero_indices[valid_mask]
            patch_idx = valid_idx[
                torch.randperm(valid_idx.shape[0])[:patch_num_per_mask]
            ]  # [K, 3]

            if patch_idx.shape[0] < patch_num_per_mask:
                patches_this_img = torch.zeros(
                    patch_num_per_mask,
                    3,
                    self.few_shot_gan_tex_patch,
                    self.few_shot_gan_tex_patch,
                ).to(self.device)
            else:
                patches_this_img = []
                ...
                for idx in range(patch_idx.shape[0]):
                    ...
                    patch = F.interpolate(
                        ...
                        size=self.few_shot_gan_tex_patch,
                        mode="bilinear",
                    )  # [1, 3, ps, ps]
    ...
    def compute_gan_tex_loss(
        ...
        proj = (
            util.perspective(self.crop_fov_approx / 180 * np.pi, 1, n=0.1, f=1000.0)
            .repeat(b, 1, 1)
            .to(self.device)
        )
        ...
        campos = torch.matmul(
            delta_rot_matrix[:, :3, :3].transpose(2, 1), original_campos[:, :, None]
        )[:, :, 0]
        w2c = w2c_pred
        ...
        image_pred, mask_pred, _, _, _, _ = self.render(
            ...
            num_frames=num_frames,
            im_features_map={
                "original_mvp": original_mvp,
                "im_features_map": im_features_map,
            }
            if im_features_map is not None
            else None,  # in other views we need to pass the original mvp
        )

        mask_pred = mask_pred.unsqueeze(1)
        if self.few_shot_gan_tex_reso != self.out_image_size:
            image_pred = torch.nn.functional.interpolate(
                image_pred, (self.out_image_size, self.out_image_size), mode="bilinear"
            )
            mask_pred = torch.nn.functional.interpolate(
                mask_pred, (self.out_image_size, self.out_image_size), mode="bilinear"
            )
        ...
        if background_for_reverse == "random":
            ...
            random_bg_gt = torch.nn.functional.interpolate(
                random_bg_gt.squeeze(1),
                (self.out_image_size, self.out_image_size),
                mode="bilinear",
            ).unsqueeze(1)
            mask_gt = mask_gt.unsqueeze(2).repeat(1, 1, 3, 1, 1)
            image_gt = image_gt * mask_gt + random_bg_gt * (1.0 - mask_gt)
            ...
            random_bg = (
                random_values[..., None, None, None, None]
                .repeat(1, 1, 3, self.few_shot_gan_tex_reso, self.few_shot_gan_tex_reso)
                .to(self.device)
            )
            random_bg = torch.nn.functional.interpolate(
                random_bg.squeeze(1),
            ...
            random_bg_gt = (
                random_values_gt[..., None, None, None, None]
                .repeat(1, 1, 3, self.few_shot_gan_tex_reso, self.few_shot_gan_tex_reso)
                .to(self.device)
            )
            random_bg_gt = torch.nn.functional.interpolate(
        ...
        if self.cfgs.get("few_shot_gan_tex_patch", 0) > 0:
            patch_size = torch.randint(
                self.few_shot_gan_tex_patch, self.few_shot_gan_tex_patch_max, (1,)
            ).item()
            # random view
            image_pred = self.generate_patch_crop(
                image_pred, mask_pred, patch_size, self.few_shot_gan_tex_patch_num
            )
            # input view
            iv_image_pred = self.generate_patch_crop(
                iv_image_pred,
                iv_mask_pred.squeeze(1)[:, 0:1, :, :],
                patch_size,
                self.few_shot_gan_tex_patch_num,
            )
            # gt view
            image_gt = self.generate_patch_crop(
                image_gt,
                mask_gt.squeeze(1)[:, 0:1, :, :],
                patch_size,
                self.few_shot_gan_tex_patch_num,
            )
        ...
        if self.few_shot_clip_tex:
            clip_tex_loss_rv_iv = self.compute_clip_loss(
                image_pred, iv_image_pred.unsqueeze(1), category="none"
            )
            clip_tex_loss_rv_gt = self.compute_clip_loss(
                image_pred, image_gt.unsqueeze(1), category="none"
            )
            clip_tex_loss = (
                clip_tex_loss_rv_iv["clip_all_loss"]
                + clip_tex_loss_rv_gt["clip_all_loss"]
            )
    ...
    def compute_mask_distribution_loss(
        self,
        category,
        w2c_pred,
        shape,
        ...
        if self.random_mask_law == "batch_swap":
            ...
            rand_degree_2 = torch.randperm(
                w2c_pred.shape[0] - int(w2c_pred.shape[0] // 2)
            ) + int(w2c_pred.shape[0] // 2)
            rand_degree = (
                torch.cat([rand_degree_2, rand_degree_1], dim=0)
                .long()
                .to(w2c_pred.device)
            )
            ...
            w2c = torch.FloatTensor(np.diag([1.0, 1.0, 1.0, 1]))
            ...
            campos = torch.matmul(
                delta_rot_matrix[:, :3, :3].transpose(2, 1), campos[:, :, None]
            )[:, :, 0]
        ...
        mask_distribution = (
            torch.Tensor(self.class_mask_distribution["zebra"])
            .to(self.device)
            .unsqueeze(0)
            .repeat(b, 1, 1)
        )
        ...
        distribution_loss = torch.nn.functional.binary_cross_entropy(
            mask_pred, mask_distribution
        )
    ...
    def use_line_correct_valid_mask(self, mask_valid, p1, p2, mvp, mask_gt):
        ...
        line_uv = line_uv * torch.Tensor(
            [mask_valid.shape[-2] // 2, mask_valid.shape[-1] // 2]
        ).to(line_uv.device).unsqueeze(0).unsqueeze(-1).repeat(b, 1, n_uv)
        line_uv = line_uv + torch.Tensor(
            [mask_valid.shape[-2] // 2, mask_valid.shape[-1] // 2]
        ).to(line_uv.device).unsqueeze(0).unsqueeze(-1).repeat(b, 1, n_uv)
        ...
        set_trace()
        line_slope = (line_uv[:, 0, 1] - line_uv[:, 1, 1]) / (
            line_uv[:, 0, 0] - line_uv[:, 1, 0]
        )

        uv = np.mgrid[0 : mask_valid.shape[-2], 0 : mask_valid.shape[-1]].astype(
            np.int32
        )
        uv = (
            torch.from_numpy(np.flip(uv, axis=0).copy())
            .float()
            .unsqueeze(0)
            .repeat(b, 1, 1, 1)
        )  # [B, 2, 256, 256]
    ...
    def discriminator_step(self):
        ...
        disc_loss = discriminator_architecture.bce_loss_target(
            d_random_pred, 0
        )  # in gen loss, train it to be real
        ...
        if self.disc_gt:
            ...
            if d_gt.requires_grad is False:
                ...
                grad_penalty = (
                    self.disc_reg_mul
                    * discriminator_architecture.compute_grad2(d_gt, mask_gt)
                )
                disc_gt_loss = (
                    discriminator_architecture.bce_loss_target(d_gt, 1) + grad_penalty
                )
        ...
        if self.disc_iv:
            ...
            if self.disc_iv_label == "Real":
                if d_iv.requires_grad is False:
                    ...
                    grad_penalty = (
                        self.disc_reg_mul
                        * discriminator_architecture.compute_grad2(d_iv, mask_pred)
                    )
                    disc_iv_loss = (
                        discriminator_architecture.bce_loss_target(d_iv, 1)
                        + grad_penalty
                    )
                    grad_loss += grad_penalty
    ...
    def compute_mask_disc_loss_gen(
        self,
        mask_gt,
        mask_pred,
        mask_random_pred,
        ...
        if not self.mask_disc_feat_condition:
            try:
                class_idx = list(self.netPrior.category_id_map.keys()).index(
                    category_name
                )
            except:
                class_idx = 100
            ...
            class_one_hot = (
                torch.nn.functional.one_hot(class_idx, num_classes=num_classes)
                .unsqueeze(-1)
                .unsqueeze(-1)
                .to(mask_gt.device)
            )
            class_one_hot = class_one_hot.repeat(
                mask_gt.shape[0], 1, mask_gt.shape[-2], mask_gt.shape[-1]
            )
            # TODO: a hack try here
            class_one_hot = class_one_hot[:, : (self.mask_disc.in_dim - 1), :, :]
        else:
            class_one_hot = condition_feat.detach()
            class_one_hot = class_one_hot.reshape(1, -1, 1, 1).repeat(
                mask_gt.shape[0], 1, mask_gt.shape[-2], mask_gt.shape[-1]
            )
        ...
        if self.disc_iv:
            if self.disc_iv_label != "Real":  # consider the input view also fake
                d_iv = self.mask_disc(mask_pred)
                disc_iv_loss = discriminator_architecture.bce_loss_target(
                    d_iv, 1
                )  # so now we need to train them to be real
                disc_loss = disc_loss + disc_iv_loss
                count = count + 1
    ...
    def forward(
        ...
        batch = [
            x.to(self.device) if x is not None and isinstance(x, torch.Tensor) else x
            for x in batch
        ]
        (
            input_image,
        ...
        if dino_cluster_im.dim() == 2:  # dummy tensor for not loading dino clusters
            ...
            dino_cluster_im_gt = expandF(
                torch.nn.functional.interpolate(
                    collapseF(dino_cluster_im), size=[h, w], mode="nearest"
                )
            )
        ...
        if self.out_image_size != self.in_image_size:
            image_gt = expandF(
                torch.nn.functional.interpolate(
                    collapseF(image_gt), size=[h, w], mode="bilinear"
                )
            )
            if flow_gt is not None:
                flow_gt = torch.nn.functional.interpolate(
                    flow_gt.view(batch_size * (num_frames - 1), 2, h0, w0),
                    size=[h, w],
                    mode="bilinear",
                ).view(batch_size, num_frames - 1, 2, h, w)
        ...
        if epoch in self.pose_xflip_recon_epochs:
            ...
            input_image_xflip_flag = torch.randint(
                0, 2, (batch_size, num_frames), device=input_image.device
            )
            input_image = (
                input_image * (1 - input_image_xflip_flag[:, :, None, None, None])
                + input_image_xflip * input_image_xflip_flag[:, :, None, None, None]
            )
        else:
            input_image_xflip_flag = None
        ...
        if self.enable_prior:
            if self.world_size > 1:
                if epoch < self.dmtet_grid_smaller_epoch:
                    if (
                        self.netPrior_ddp.module.netShape.grid_res
                        != self.dmtet_grid_smaller
                    ):
                        self.netPrior_ddp.module.netShape.load_tets(
                            self.dmtet_grid_smaller
                        )
                else:
            ...
            if self.world_size > 1:
                prior_shape, dino_pred, classes_vectors = self.netPrior_ddp(
                ...
                prior_shape, dino_pred, classes_vectors = self.netPrior(
                    category_name=category_name[0],
                    perturb_sdf=perturb_sdf,
                    total_iter=total_iter,
                    is_training=is_training,
        ...
        if self.calc_dino_features == True:
            ...
            (
                ...
                width,
                channels,
            ) = dino_feat_im_calc.shape  # 3 X 384 X 32 X 32
        ...
        if dino_feat_im.dim() == 2:  # dummy tensor for not loading dino features
            ...
            dino_feat_im_gt = expandF(
                torch.nn.functional.interpolate(
                    collapseF(dino_feat_im), size=[h, w], mode="bilinear"
                )
            )[:, :, : self.dino_feature_recon_dim]
        ...
        if classes_vectors is not None:
            if len(classes_vectors.shape) == 1:
                ...
                class_vector = classes_vectors[
                    self.netPrior.category_id_map[category_name[0]], :
                ]
        ...
        image_pred, mask_pred, flow_pred, dino_feat_im_pred = map(
            expandF, (image_pred, mask_pred, flow_pred, dino_feat_im_pred)
        )
        ...
        if self.blur_mask:
            ...
            if sigma > 0.5:
                mask_gt = util.blur_image(
                    mask_gt, kernel_size=9, sigma=sigma, mode="gaussian"
                )
        ...
        for name, loss in losses.items():
            if name == "flow_loss":
                continue
            loss_weight_logit = self.cfgs.get(f"{name}_weight", 0.0)

            if isinstance(loss_weight_logit, dict):
                loss_weight_logit = self.parse_dict_definition(
                    loss_weight_logit, total_iter
                )
            ...
            if name in [
                "sdf_bce_reg_loss",
                "sdf_gradient_reg_loss",
                "sdf_inflate_reg_loss",
            ]:
                if total_iter >= self.sdf_reg_decay_start_iter:
                    decay_rate = max(
                        0, 1 - (total_iter - self.sdf_reg_decay_start_iter) / 10000
                    )
                    loss_weight_logit = max(
                        loss_weight_logit * decay_rate,
                        self.cfgs.get(f"{name}_min_weight", 0.0),
                    )
            if name in ["dino_feat_im_loss"]:
                dino_feat_im_loss_multipler = self.cfgs.get(
                    "logit_loss_dino_feat_im_loss_multiplier", 1.0
                )

                if isinstance(dino_feat_im_loss_multipler, dict):
                    dino_feat_im_loss_multipler = self.parse_dict_definition(
                        dino_feat_im_loss_multipler, total_iter
                    )
            ...
            if self.netInstance.rot_rep in ["quadlookat", "octlookat"]:
                loss = (
                    loss
                    * rot_prob.detach().view(batch_size, num_frames)[:, : loss.shape[1]]
                    * self.netInstance.num_pose_hypos
                )
        ...
        final_losses["logit_loss"] = (
            (expandF(rot_logit) - logit_loss_target.detach()) ** 2.0
        ).mean()
        ...
        if self.enable_mask_distribution:
            if total_iter % self.mask_distribution_loss_freq == 0:
                (
                    mask_distribution_loss,
                    mask_distribution_aux,
                ) = self.compute_mask_distribution_loss(
                    category_name[0],
                    w2c,
                    shape,
                    prior_shape,
                    ...
                    im_features,
                    light,
                    class_vector[None, :].expand(batch_size * num_frames, -1),
                    num_frames,
                    im_features_map,
                )
                ...
                if self.enable_clip:
                    ...
                    clip_all_loss = self.compute_clip_loss(
                        random_render_image, image_pred, category_name[0]
                    )  # a dict
        ...
        if (
            self.enable_disc
            and (self.mask_discriminator_iter[0] < total_iter)
            and (self.mask_discriminator_iter[1] > total_iter)
        ):
            disc_loss = self.compute_mask_disc_loss_gen(
                mask_gt,
                mask_pred,
                mask_distribution_aux["mask_random_pred"],
                category_name=category_name[0],
                condition_feat=class_vector,
            )
            final_losses.update(disc_loss)

        # implement the gan training for local texture in fine-tuning
        gan_tex_aux = None
        if (self.few_shot_gan_tex and viz_logger is None) or (
            self.few_shot_gan_tex
            and viz_logger is not None
            and logger_prefix == "train_"
        ):
            gan_tex_loss, gan_tex_aux = self.compute_gan_tex_loss(
                category_name[0],
                image_gt,
                mask_gt,
                image_pred,
        ...
        if bank_embedding is not None:
            ...
            bank_mean_dist = torch.nn.functional.mse_loss(
                embeddings, batch_embedding.unsqueeze(0).repeat(batch_size, 1)
            )
        ...
        for name, loss in final_losses.items():
            loss_weight = self.cfgs.get(f"{name}_weight", 0.0)
            ...
            if epoch not in self.lookat_zflip_loss_epochs:
                if name in ["lookat_zflip_loss"]:
                    continue
            if name in [
                "mesh_laplacian_smoothing_loss",
                "mesh_normal_consistency_loss",
            ]:
                if total_iter < self.cfgs.get("mesh_reg_start_iter", 0):
                    continue
                if epoch >= self.mesh_reg_decay_epoch:
                    decay_rate = self.mesh_reg_decay_rate ** (
                        epoch - self.mesh_reg_decay_epoch
                    )
                    loss_weight = max(
                        loss_weight * decay_rate,
                        self.cfgs.get(f"{name}_min_weight", 0.0),
                    )
        ...
        if viz_logger is not None:
            ...
            viz_logger.add_image(
                logger_prefix + "image/image_gt",
                misc.image_grid(
                    image_gt.detach()
                    .cpu()[:b0, :]
                    .reshape(-1, *input_image.shape[2:])
                    .clamp(0, 1)
                ),
                total_iter,
            )
            viz_logger.add_image(
                logger_prefix + "image/image_pred",
                misc.image_grid(
                    image_pred.detach()
                    .cpu()[:b0, :]
                    .reshape(-1, *image_pred.shape[2:])
                    .clamp(0, 1)
                ),
                total_iter,
            )
            ...
            viz_logger.add_image(
                logger_prefix + "image/mask_gt",
                misc.image_grid(
                    mask_gt.detach()
                    .cpu()[:b0, :]
                    .reshape(-1, *mask_gt.shape[2:])
                    .unsqueeze(1)
                    .repeat(1, 3, 1, 1)
                    .clamp(0, 1)
                ),
            ...
            viz_logger.add_image(
                logger_prefix + "image/mask_pred",
                misc.image_grid(
                    mask_pred.detach()
                    .cpu()[:b0, :]
                    .reshape(-1, *mask_pred.shape[2:])
                    .unsqueeze(1)
                    .repeat(1, 3, 1, 1)
                    .clamp(0, 1)
                ),
            ...
            if self.render_flow and flow_gt is not None:
                ...
                flow_gt_viz = (
                    torch.cat([flow_gt[:b0], torch.zeros_like(flow_gt[:b0, :, :1])], 2)
                    + 0.5
                )  # -0.5~1.5
                flow_gt_viz = torch.nn.functional.pad(
                    flow_gt_viz, pad=[0, 0, 0, 0, 0, 0, 0, 1]
                )
                ...
                viz_logger.add_image(
                    logger_prefix + "image/flow_gt",
                    misc.image_grid(flow_gt_viz.reshape(-1, *flow_gt_viz.shape[2:])),
                    total_iter,
                )
            ...
            if self.render_flow and flow_pred is not None:
                ...
                flow_pred_viz = (
                    torch.cat(
                        [flow_pred[:b0], torch.zeros_like(flow_pred[:b0, :, :1])], 2
                    )
                    + 0.5
                )  # -0.5~1.5
                flow_pred_viz = torch.nn.functional.pad(
                    flow_pred_viz, pad=[0, 0, 0, 0, 0, 0, 0, 1]
                )
                viz_logger.add_image(
                    logger_prefix + "image/flow_pred",
                    misc.image_grid(
                        flow_pred_viz.reshape(-1, *flow_pred_viz.shape[2:])
                    ),
                    total_iter,
                )
            ...
            if mask_distribution_aux is not None:
                ...
                mask_random_pred = (
                    mask_distribution_aux["mask_random_pred"].detach().cpu().clamp(0, 1)
                )
                mask_distribution_data = (
                    mask_distribution_aux["mask_distribution"]
                    .detach()
                    .cpu()
                    .clamp(0, 1)
                )
                ...
                mask_random_pred_image = [
                    misc.add_text_to_image(img, str(text.item()))
                    for img, text in zip(mask_random_pred, degree_text)
                ]
            ...
            if gan_tex_aux is not None:
                gan_tex_render_image = (
                    gan_tex_aux["gan_tex_render_image"].detach().cpu().clamp(0, 1)
                )
                ...
                gan_tex_render_image_iv = (
                    gan_tex_aux["gan_tex_inpview_image"].detach().cpu().clamp(0, 1)
                )
                ...
                gan_tex_render_image_gt = (
                    gan_tex_aux["gan_tex_gt_image"].detach().cpu().clamp(0, 1)
                )
            ...
            if light is not None:
                ...
                for name, param in zip(param_names, light.light_params.unbind(-1)):
                    viz_logger.add_histogram(
                        logger_prefix + "light/" + name, param, total_iter
                    )
                viz_logger.add_image(
                    logger_prefix + f"image/albedo",
                    misc.image_grid(
                        expandF(albedo)[:b0, ...].view(-1, *albedo.shape[1:])
                    ),
                    total_iter,
                )
                viz_logger.add_image(
            ...
            viz_logger.add_histogram(
                logger_prefix + "sdf",
                self.netPrior.netShape.get_sdf(
                    perturb_sdf=False, class_vector=class_vector
                ),
                total_iter,
            )
            viz_logger.add_histogram(
                logger_prefix + "coordinates", shape.v_pos, total_iter
            )
            if arti_params is not None:
                viz_logger.add_histogram(
                    logger_prefix + "arti_params", arti_params, total_iter
                )
                viz_logger.add_histogram(
                    logger_prefix + "edge_lengths", aux_viz["edge_lengths"], total_iter
                )

            if deformation is not None:
                viz_logger.add_histogram(
                    logger_prefix + "deformation", deformation, total_iter
                )
            ...
            if rot_rep == "euler_angle" or rot_rep == "soft_calss":
                for i, name in enumerate(
                    ...
                    viz_logger.add_histogram(
                        logger_prefix + "pose/" + name, pose[..., i], total_iter
                    )
            elif rot_rep == "quaternion":
                for i, name in enumerate(
                    ["qt_0", "qt_1", "qt_2", "qt_3", "trans_x", "trans_y", "trans_z"]
                ):
                    viz_logger.add_histogram(
                        logger_prefix + "pose/" + name, pose[..., i], total_iter
                    )
                rot_euler = pytorch3d.transforms.matrix_to_euler_angles(
            ...
            if rot_rep in ["quadlookat", "octlookat"]:
                for i, rp in enumerate(forward_aux["rots_probs"].unbind(-1)):
                    viz_logger.add_histogram(
                        logger_prefix + "pose/rot_prob_%d" % i, rp, total_iter
                    )
            ...
            if bank_embedding is not None:
                ...
                for i, weight_for_emb in enumerate(weights_for_emb.unbind(-1)):
                    viz_logger.add_histogram(
                        logger_prefix + "bank_embedding/emb_weight_%d" % i,
                        weight_for_emb,
                        total_iter,
                    )
                ...
                for i, idx_for_emb in enumerate(indices_for_emb.unbind(-1)):
                    viz_logger.add_histogram(
                        logger_prefix + "bank_embedding/emb_idx_%d" % i,
                        idx_for_emb,
                        total_iter,
                    )
            ...
            if dino_feat_im_gt is not None:
                ...
                viz_logger.add_image(
                    logger_prefix + "image/dino_feat_im_gt",
                    misc.image_grid(
                        dino_feat_im_gt_first3.detach()
                        .cpu()[:b0, :]
                        .reshape(-1, *dino_feat_im_gt_first3.shape[2:])
                        .clamp(0, 1)
                    ),
                    total_iter,
                )
            ...
            if dino_cluster_im_gt is not None:
                viz_logger.add_image(
                    logger_prefix + "image/dino_cluster_im_gt",
                    misc.image_grid(
                        dino_cluster_im_gt.detach()
                        .cpu()[:b0, :]
                        .reshape(-1, *dino_cluster_im_gt.shape[2:])
                        .clamp(0, 1)
                    ),
                    total_iter,
                )
            ...
            if dino_feat_im_pred is not None:
                ...
                viz_logger.add_image(
                    logger_prefix + "image/dino_feat_im_pred",
                    misc.image_grid(
                        dino_feat_im_pred_first3.detach()
                        .cpu()[:b0, :]
                        .reshape(-1, *dino_feat_im_pred_first3.shape[2:])
                        .clamp(0, 1)
                    ),
                    total_iter,
                )
            ...
            for which_shape, modes in self.extra_renders.items():
                ...
                for mode in modes:
                    if mode in ["gray"]:
                        gray_light = FixedDirectionLight(
                            direction=torch.FloatTensor([0, 0, 1]).to(self.device),
                            amb=0.2,
                            diff=0.7,
                        )
                        _, render_mask, _, _, _, rendered = self.render(
                            shape_to_render,
                            texture,
                            mvp,
                            w2c,
                            ...
                            (h, w),
                            background=self.background_mode,
                            im_features=needed_im_features,
                            prior_shape=prior_shape,
                            render_mode="diffuse",
                            light=gray_light,
                            render_flow=False,
                        ...
                        rendered, _, _, _, _, _ = self.render(
                            shape_to_render,
                            texture,
                            mvp,
                            w2c,
                    ...
                    if "posed_bones" in aux_viz:
                        rendered_bone_image = self.render_bones(
                            mvp, aux_viz["posed_bones"], (h, w)
                        )
                        rendered_bone_image_mask = (
                            (rendered_bone_image < 1).any(1, keepdim=True).float()
                        )
                        ...
                        rendered = (
                            rendered_bone_image_mask * 0.8 * rendered_bone_image
                            + (1 - rendered_bone_image_mask * 0.8) * rendered
                        )

                    if rot_rep in ["quadlookat", "octlookat"]:
                        rand_pose_flag = forward_aux["rand_pose_flag"].detach().cpu()
                        rand_pose_marker_mask = torch.zeros_like(rendered)
                        rand_pose_marker_mask[:, :, :16, :16] = 1.0
                        rand_pose_marker_mask = (
                            rand_pose_marker_mask * rand_pose_flag[:, None, None, None]
                        )
                        red = torch.FloatTensor([1, 0, 0])[None, :, None, None]
                        rendered = (
                            rand_pose_marker_mask * red
                            + (1 - rand_pose_marker_mask) * rendered
                        )

                    viz_logger.add_image(
                        logger_prefix + f"image/{which_shape}_{mode}",
                        misc.image_grid(
                            expandF(rendered)[:b0, ...].view(-1, *rendered.shape[1:])
                        ),
                        total_iter,
                    )
                    ...
                    if rendered_wo_bones is not None:
                        viz_logger.add_image(
                            ...
                            misc.image_grid(
                                expandF(rendered_wo_bones)[:b0, ...].view(
                                    -1, *rendered_wo_bones.shape[1:]
                                )
                            ),
                            total_iter,
                        )
        ...
        if save_results and self.rank == 0:
            ...
            # from IPython import embed; embed()
            fnames = [
                f"{total_iter:07d}_{fid:010d}" for fid in collapseF(frame_id.int())
            ][:b0]
            ...
            misc.save_images(
                save_dir,
                collapseF(image_gt)[:b0].clamp(0, 1).detach().cpu().numpy(),
                suffix="image_gt",
                fnames=fnames,
            )
            misc.save_images(
                save_dir,
                collapseF(image_pred)[:b0].clamp(0, 1).detach().cpu().numpy(),
                suffix="image_pred",
                fnames=fnames,
            )
            misc.save_images(
                save_dir,
                collapseF(mask_gt)[:b0]
                .unsqueeze(1)
                .repeat(1, 3, 1, 1)
                .clamp(0, 1)
                .detach()
            ...
            if self.render_flow and flow_gt is not None:
                flow_gt_viz = (
                    torch.cat([flow_gt, torch.zeros_like(flow_gt[:, :, :1])], 2) + 0.5
                )  # -0.5~1.5
                flow_gt_viz = flow_gt_viz.view(-1, *flow_gt_viz.shape[2:])
                misc.save_images(
                    save_dir,
                    flow_gt_viz[:b0].clamp(0, 1).detach().cpu().numpy(),
                    suffix="flow_gt",
                    fnames=fnames,
                )
            if flow_pred is not None:
                flow_pred_viz = (
                    torch.cat([flow_pred, torch.zeros_like(flow_pred[:, :, :1])], 2)
                    + 0.5
                )  # -0.5~1.5
                flow_pred_viz = flow_pred_viz.view(-1, *flow_pred_viz.shape[2:])
                misc.save_images(
                    save_dir,
                    flow_pred_viz[:b0].clamp(0, 1).detach().cpu().numpy(),
                    suffix="flow_pred",
                    fnames=fnames,
                )

            misc.save_txt(
                save_dir, pose[:b0].detach().cpu().numpy(), suffix="pose", fnames=fnames
            )
        return metrics
    ...
    def render_rotation_frames(
        ...
        if b is None:
            ...
            im_features_map = (
                im_features_map[:b] if im_features_map is not None else None
            )
            original_mvp = (
                original_mvp[:b] if original_mvp is not None else None
            )  # [b, 4, 4]
        ...
        if (
            original_w2c is not None
            and original_campos is not None
            and original_mvp is not None
        ):
            w2c = original_w2c[:b]
        ...
        for _ in range(num_frames):
            if render_gray:
                _, render_mask, _, _, _, image_pred = self.render(
                    mesh,
                    texture,
                    mvp,
                    w2c,
                ...
                image_pred, _, _, _, _, _ = self.render(
                    mesh,
                    texture,
                    mvp,
                    w2c,
        ...
        if text is not None:
            frames = [
                torch.Tensor(misc.add_text_to_image(f, text)).permute(2, 0, 1)
                for f in frames
            ]
    ...
    def render_bones(self, mvp, bones_pred, size=(256, 256)):
        bone_world4 = torch.concat(
            [bones_pred, torch.ones_like(bones_pred[..., :1]).to(bones_pred.device)],
            dim=-1,
        )
        b, f, num_bones = bone_world4.shape[:3]
        bones_clip4 = (
            bone_world4.view(b, f, num_bones * 2, 1, 4)
            @ mvp.transpose(-1, -2).reshape(b, f, 1, 4, 4)
        ...
        for b_idx in range(b):
            for f_idx in range(f):
                ...
                for bone in frame_bones_uv:
                    ax.plot(
                        bone[:, 0], bone[:, 1], marker="o", linewidth=8, markersize=20
                    )
                ax.set_xlim(-1, 1)
                ax.set_ylim(-1, 1)
                ax.invert_yaxis()
    ...
    def vis_sds_image(self, sds_image, sds_aux):
        ...
        sds_image = [
            misc.add_text_to_image(img, text)
            for img, text in zip(sds_image, sds_aux["dirs"])
        ]
