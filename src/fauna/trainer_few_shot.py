...
import src.fauna.utils.meters as meters
import src.fauna.utils.misc as misc
from src.fauna import discriminator_architecture

# from video3d.dataloaders import get_image_loader
from src.fauna.dataloaders_ddp import (
    get_sequence_loader_ddp,
    get_sequence_loader_quadrupeds,
    get_test_loader_quadrupeds,
)
...
def sample_frames(batch, num_sample_frames, iteration, stride=1):
    ...
    new_batch = (
        images[:, start_frame_idx : start_frame_idx + num_sample_frames],
...
class Fewshot_Trainer:
    def __init__(self, cfgs, model):
        ...
        self.keep_num_checkpoint = cfgs.get(
            "keep_num_checkpoint", 2
        )  # -1 for keeping all checkpoints
        ...
        assert (
            "data_resize_update" in self.few_shot_data_dir
        )  # TODO: a hack way to make sure not using wrong data, needs to remove
        (
            self.few_shot_categories,
            self.few_shot_categories_paths,
        ) = self.parse_few_shot_categories(
            self.few_shot_data_dir, self.few_shot_data_dir_more
        )
        ...
        if self.test_category_num > 0:
            ...
            if self.test_category_names is not None:
                ...
                test_cats = list(self.few_shot_categories_paths.keys())[
                    -(self.test_category_num) :
                ]
            test_categories_paths = {}
            for test_cat in test_cats:
                test_categories_paths.update(
                    {test_cat: self.few_shot_categories_paths[test_cat]}
                )
        ...
        if self.combine_dataset:
            (
                ...
                self.val_loader,
                self.test_loader,
            ) = self.get_data_loaders_quadrupeds(
                self.cfgs,
                self.batch_size,
                self.num_workers,
                self.in_image_size,
            ...
            (
                self.train_loader_few_shot,
                self.val_loader_few_shot,
            ) = self.get_data_loaders_few_shot(
                self.cfgs,
                self.batch_size,
                self.num_workers,
                self.in_image_size,
            ...
            ) = self.get_data_loaders_original(
                self.cfgs,
                self.batch_size,
                self.num_workers,
                self.in_image_size,
            ...
            if (
                self.val_loader_few_shot is not None
                and self.val_loader_original is not None
            ):
        ...
        if self.use_total_iterations:
            ...
            dataloader_length = max(
                [len(loader) for loader in self.train_loader]
        ...
        if self.enable_memory_bank:
            ...
            if self.memory_bank_init == "copy":
                ...
                self.memory_bank = torch.nn.Parameter(
                    torch.nn.init.uniform_(
                        torch.empty(self.memory_bank_size, self.memory_bank_dim),
                        a=-0.05,
                        b=0.05,
                    ),
                    requires_grad=True,
            ...
            self.memory_encoder = cfgs.get(
                "memory_encoder", "DINO"
            )  # if DINO then just use the network encoder
            ...
            memory_bank_keys = torch.nn.init.uniform_(
                torch.empty(self.memory_bank_size, self.memory_bank_keys_dim),
                a=-0.05,
                b=0.05,
            )
            self.memory_bank_keys = torch.nn.Parameter(
                memory_bank_keys, requires_grad=True
            )

        else:
            print(
                "no memory bank, just use image embedding, this is only for one experiment!"
            )
            self.memory_encoder = cfgs.get(
                "memory_encoder", "DINO"
            )  # if DINO then just use the network encoder
            ...
            for data_dir_one in data_dir_more:
                ...
                for new_category in new_categories:
                    ...
                    if new_category not in category_names:
                        ...
                        if new_category in list(
                            self.cfgs.get("train_data_dir", None).keys()
                        ):
                        ...
                        category_names_paths.update(
                            {
                                new_category: osp.join(
                                    data_dir_one, new_category, "train"
                                )
                            }
                        )
                    else:
    ...
    def prepare_model(self):
        ...
        if self.cfgs.get("few_shot_checkpoint_name", None) is not None:
            original_checkpoint_path = osp.join(
                self.checkpoint_dir,
                self.cfgs.get("few_shot_checkpoint_name", "checkpoint060.pth"),
            )
            ...
            if (self.cfgs.get("texture_way", None) is not None) or (
            ...
            self.model.netInstance.load_state_dict(
                cp["netInstance"], strict=False
            )  # For Deform
            ...
            print(
                "not load any pre-train weight, the iter will start from 0, make sure you set all the needed parameters"
            )
        ...
        if not self.cfgs.get("disable_fewshot", False):
            ...
            few_shot_class_vector_init = self.cfgs.get(
                "few_shot_class_vector_init", "random"
            )
            if few_shot_class_vector_init == "random":
                tmp = torch.nn.init.uniform_(
                    torch.empty(
                        self.new_classes_num,
                        self.model.netPrior.classes_vectors.shape[-1],
                    ),
                    a=-0.05,
                    b=0.05,
                )
                tmp = tmp.to(self.model.netPrior.classes_vectors.device)
                self.model.netPrior.classes_vectors = torch.nn.Parameter(
                    torch.cat([self.model.netPrior.classes_vectors, tmp], dim=0)
                )
            elif few_shot_class_vector_init == "copy":
                num_7_cat_piece = (
                    self.new_classes_num // self.original_classes_num
                    if self.new_classes_num > self.original_classes_num
                    else 0
                )
                num_left = (
                    self.new_classes_num - num_7_cat_piece * self.original_classes_num
                )
                ...
                if num_7_cat_piece > 0:
                    ...
                    tmp_1 = tmp_1.reshape(
                        tmp_1.shape[0] * tmp_1.shape[1], tmp_1.shape[-1]
                    )
                else:
                    tmp_1 = None
        ...
        if optimize_rule == "all":
            optimize_list = [
                {
                    "name": "net_Prior",
                    "params": list(self.model.netPrior.parameters()),
                    "lr": self.lr * 10.0,
                },
                {
                    "name": "net_Instance",
                    "params": list(self.model.netInstance.parameters()),
                    "lr": self.lr * 1.0,
                },
            ]
            ...
            optimize_list = [
                {
                    ...
                    "params": list([self.model.netPrior.classes_vectors]),
                    "lr": self.lr * 10.0,
                }
            ]
            ...
            optimize_list = [
                ...
                {
                    "name": "netEncoder",
                    "params": list(self.model.netInstance.netEncoder.parameters()),
                    "lr": self.lr * 1.0,
                },
                {
                    "name": "netTexture",
                    "params": list(self.model.netInstance.netTexture.parameters()),
                    "lr": self.lr * 1.0,
                },
                {
                    "name": "netPose",
                    "params": list(self.model.netInstance.netPose.parameters()),
                    "lr": self.lr * 0.01,
                },
                {
                    "name": "netArticulation",
                    "params": list(self.model.netInstance.netArticulation.parameters()),
                    "lr": self.lr * 1.0,
                },
                {
                    "name": "netLight",
                    "params": list(self.model.netInstance.netLight.parameters()),
                    "lr": self.lr * 1.0,
                },
            ]
            ...
            optimize_list = [
                ...
                {
                    "name": "netDeform",
                    "params": list(self.model.netInstance.netDeform.parameters()),
                    "lr": self.lr * 1.0,
                },
            ]
        ...
        if self.enable_memory_bank and optimize_rule != "texture":
            ...
            if optimize_bank_components == "value":
                ...
                optimize_list += [
                    {
                        "name": "memory_bank_keys",
                        "params": list([self.memory_bank_keys]),
                        "lr": self.lr * 10.0,
                    }
                ]
            else:
        ...
        if self.model.enable_vsd:
            optimize_list += [
                {
                    "name": "lora",
                    "params": list(self.model.stable_diffusion.parameters()),
                    "lr": self.lr,
                }
            ]
        ...
        # )
        self.optimizerFewShot = torch.optim.Adam(
            optimize_list, betas=(0.9, 0.99), eps=1e-15
        )

        # if self.cfgs.get('texture_way', None) is not None and self.cfgs.get('gan_tex', False):
        if self.cfgs.get("gan_tex", False):
            self.optimizerDiscTex = torch.optim.Adam(
                filter(
                    lambda p: p.requires_grad,
                    self.model.discriminator_texture.parameters(),
                ),
                lr=self.lr,
                betas=(0.9, 0.99),
                eps=1e-15,
    ...
    def load_checkpoint(self, optim=True, checkpoint_name=None):
        ...
        if optim:
            try:
                ...
                print(
                    "you should be using the local texture so dont need to load the previous optimizer"
                )
        if self.enable_memory_bank:
    ...
    def save_checkpoint(self, epoch, total_iter=0, optim=True, use_iter=False):
        ...
        if self.keep_num_checkpoint > 0:
            self.clean_checkpoint(
                self.checkpoint_dir, keep_num=self.keep_num_checkpoint, prefix=prefix
            )

    def clean_checkpoint(self, checkpoint_dir, keep_num=2, prefix="checkpoint*.pth"):
        if keep_num > 0:
            names = list(sorted(glob.glob(os.path.join(checkpoint_dir, prefix))))
            if len(names) > keep_num:
                for name in names[:-keep_num]:
                    print(f"Deleting obslete checkpoint file {name}")
                    os.remove(name)

    def get_data_loaders_few_shot(
        self, cfgs, batch_size, num_workers, in_image_size, out_image_size
    ):
        ...
        train_loader = get_loader_ddp(
            data_dir=[self.original_classes_num, self.few_shot_categories_paths],
            rank=self.rank,
    ...
    def get_data_loaders_quadrupeds(
        self, cfgs, batch_size, num_workers, in_image_size, out_image_size
    ):
        train_loader = val_loader = test_loader = None
        color_jitter_train = cfgs.get("color_jitter_train", None)
        ...
        if self.test_categories_paths is not None:
            ...
            test_loader = get_test_loader_ddp(
                test_data_dirs=self.test_categories_paths,
                rank=self.rank,
                world_size=self.world_size,
                batch_size=1,
                is_validation=True,
    ...
    def forward_frozen_ViT(self, images):
        ...
        with torch.no_grad():
            ...
            ph, pw = (
                h // self.model.netInstance.netEncoder.patch_size,
                w // self.model.netInstance.netEncoder.patch_size,
            )
            patch_out = out[:, 1:]  # first is class token
            patch_out = patch_out.reshape(
                b, ph, pw, self.model.netInstance.netEncoder.vit_feat_dim
            ).permute(0, 3, 1, 2)

            patch_key = self.model.netInstance.netEncoder._feats[0][
                :, :, 1:
            ]  # B, num_heads, num_patches, dim
            patch_key = patch_key.permute(0, 1, 3, 2).reshape(
                b, self.model.netInstance.netEncoder.vit_feat_dim, ph, pw
            )
    ...
    def forward_fix_embeddings(self, batch):
        ...
        if self.memory_encoder == "DINO":
            ...
            images_in = torch.nn.functional.interpolate(
                images, (self.clip_reso, self.clip_reso), mode="bilinear"
            )
    ...
    def retrieve_memory_bank(self, batch_features, batch):
        ...
        if self.memory_retrieve == "cos-linear":
            ...
            cos_dist = torch.bmm(query, key).squeeze(
                1
            )  # [B, size], larger the more similar
            rank_idx = torch.sort(cos_dist, dim=-1, descending=True)[1][
                :, : self.memory_bank_topk
            ]  # [B, k]
            value = (
                self.memory_bank.unsqueeze(0).repeat(batch_size, 1, 1).to(query.device)
            )  # [B, size, d_v]

            out = torch.gather(
                value,
                dim=1,
                index=rank_idx[..., None].repeat(1, 1, self.memory_bank_dim),
            )  # [B, k, d_v]
            ...
            weights = (
                ...
                .unsqueeze(-1)
                .repeat(1, 1, self.memory_bank_dim)
    ...
    def discriminator_texture_step(self):
        ...
        if "gt" in self.model.few_shot_gan_tex_real:
            ...
            if image_gt.requires_grad:
                grad_penalty_gt = 10.0 * discriminator_architecture.compute_grad2(
                    d_gt, image_gt
                )
        ...
        if "iv" in self.model.few_shot_gan_tex_real:
            ...
            if image_iv.requires_grad:
                grad_penalty_iv = 10.0 * discriminator_architecture.compute_grad2(
                    d_iv, image_iv
                )
        ...
        self.discriminator_texture_loss = 0.0
    ...
    def train(self):
        ...
        if self.archive_code:
            misc.archive_code(
                osp.join(self.checkpoint_dir, "archived_code.zip"), filetypes=[".py"]
            )
        ...
        if self.few_shot_resume:
            ...
            start_epoch, self.total_iter = self.load_checkpoint(
                optim=True, checkpoint_name=resume_model_name
            )
        ...
        if self.use_logger:
            ...
            self.viz_data_iterator = (
                indefinite_generator(self.val_loader[0])
                if self.visualize_validation
                else indefinite_generator(self.train_loader[0])
            )
            if self.fix_viz_batch:
                self.viz_batch = next(self.viz_data_iterator)

            if self.test_loader is not None:
                self.viz_test_data_iterator = (
                    indefinite_generator(self.test_loader[0])
                    if self.visualize_validation
                    else indefinite_generator(self.train_loader[0])
                )
    ...
    def run_epoch(self, epoch):
        ...
        train_generators = [
            indefinite_generator(loader) for loader in self.train_loader
        ]
        ...
        while iteration < max_loader_len * len(self.train_loader):
            for generator in train_generators:
                ...
                if self.enable_memory_bank:
                    ...
                    batch_embedding, embeddings, weights = self.retrieve_memory_bank(
                    ...
                    weights = {
                        ...
                        "pick_idx": torch.randint(low=0, high=60, size=(1, 10)).to(
                            batch_features.device
                        ),
                    }
                    bank_embedding_model_input = [
                        batch_features[0],
                        batch_features,
                        weights,
                    ]
                m = self.model.forward(
                    ...
                    which_data=self.dataset,
                    is_training=True,
                    bank_embedding=bank_embedding_model_input,
                )
                ...
                if (
                    self.model.enable_disc
                    and (self.model.mask_discriminator_iter[0] < self.total_iter)
                    and (self.model.mask_discriminator_iter[1] > self.total_iter)
                ):
                    # the discriminator training
                    discriminator_loss_dict, grad_loss = self.model.discriminator_step()
                    m.update(
                        {
                            "mask_disc_loss_discriminator": discriminator_loss_dict[
                                "discriminator_loss"
                            ]
                            - grad_loss,
                            "mask_disc_loss_discriminator_grad": grad_loss,
                            "mask_disc_loss_discriminator_rv": discriminator_loss_dict[
                                "discriminator_loss_rv"
                            ],
                            "mask_disc_loss_discriminator_iv": discriminator_loss_dict[
                                "discriminator_loss_iv"
                            ],
                            "mask_disc_loss_discriminator_gt": discriminator_loss_dict[
                                "discriminator_loss_gt"
                            ],
                        }
                    )
                    self.logger.add_histogram(
                        "train_" + "discriminator_logits/random_view",
                        discriminator_loss_dict["d_rv"],
                        self.total_iter,
                    )
                    if discriminator_loss_dict["d_iv"] is not None:
                        self.logger.add_histogram(
                            "train_" + "discriminator_logits/input_view",
                            discriminator_loss_dict["d_iv"],
                            self.total_iter,
                        )
                    if discriminator_loss_dict["d_gt"] is not None:
                        self.logger.add_histogram(
                            "train_" + "discriminator_logits/gt_view",
                            discriminator_loss_dict["d_gt"],
                            self.total_iter,
                        )
                ...
                if (
                    self.iteration_save
                    and self.total_iter % self.iteration_save_freq == 0
                ):
                    self.save_checkpoint(
                        epoch + 1, total_iter=self.total_iter, optim=True, use_iter=True
                    )
                ...
                if misc.is_main_process() and self.use_logger:
                    if self.rank == 0 and self.total_iter % self.log_freq_losses == 0:
                        for name, loss in m.items():
                            label = (
                                f"cub_loss_train/{name[4:]}"
                                if "cub" in name
                                else f"loss_train/{name}"
                            )
                            self.logger.add_scalar(label, loss, self.total_iter)
                    if (
                        self.rank == 0
                        and self.save_result_freq is not None
                        and self.total_iter % self.save_result_freq == 0
                    ):
                        with torch.no_grad():
                            m = self.model.forward(
                                batch,
                                ...
                                total_iter=self.total_iter,
                                save_results=False,
                                save_dir=self.train_result_dir,
                                which_data=self.dataset,
                                is_training=False,
                                bank_embedding=bank_embedding_model_input,
                            )
                        ...
                        with torch.no_grad():
                            if self.rank == 0 and self.log_train_images:
                                m = self.model.forward(
                                    ...
                                    epoch=epoch,
                                    iter=iteration,
                                    viz_logger=self.logger,
                                    total_iter=self.total_iter,
                                    which_data=self.dataset,
                                    logger_prefix="train_",
                                    is_training=False,
                                    bank_embedding=bank_embedding_model_input,
                                )
                            if self.fix_viz_batch:
                            ...
                            if self.visualize_validation:
                                ...
                                #     batch = next(self.viz_data_iterator)
                                if self.enable_memory_bank:
                                    batch_features_val = self.forward_fix_embeddings(
                                        batch_val
                                    )
                                    (
                                        batch_embedding_val,
                                        embeddings_val,
                                        weights_val,
                                    ) = self.retrieve_memory_bank(
                                        batch_features_val, batch_val
                                    )
                                    bank_embedding_model_input_val = [
                                        batch_embedding_val,
                                        embeddings_val,
                                        weights_val,
                                    ]
                                    ...
                                    weights_val = {
                                        "weights": torch.rand(1, 10).to(
                                            batch_features_val.device
                                        ),
                                        "pick_idx": torch.randint(
                                            low=0, high=60, size=(1, 10)
                                        ).to(batch_features_val.device),
                                    }
                                    bank_embedding_model_input_val = [
                                        batch_features_val[0],
                                        batch_features_val,
                                        weights_val,
                                    ]
                                ...
                                if self.total_iter % self.save_result_freq == 0:
                                    m = self.model.forward(
                                        ...
                                        logger_prefix="val_",
                                        is_training=False,
                                        bank_embedding=bank_embedding_model_input_val,
                                    )
                                    torch.cuda.empty_cache()
                                ...
                                if self.test_loader is not None:
                                    ...
                                    if self.enable_memory_bank:
                                        ...
                                        (
                                            ...
                                            batch_features_test, batch_test
                                        )
                                        bank_embedding_model_input_test = [
                                            batch_embedding_test,
                                            embeddings_test,
                                            weights_test,
                                        ]
                                        ...
                                        weights_test = {
                                            "weights": torch.rand(1, 10).to(
                                                batch_features_test.device
                                            ),
                                            "pick_idx": torch.randint(
                                                low=0, high=60, size=(1, 10)
                                            ).to(batch_features_test.device),
                                        }
                                        bank_embedding_model_input_test = [
                                            batch_features_test[0],
                                            batch_features_test,
                                            weights_test,
                                        ]
                                    m_test = self.model.forward(
                                        batch_test,
                                        epoch=epoch,
                                        iter=iteration,
                                        viz_logger=self.logger,
                                        ...
                                        logger_prefix="test_",
                                        is_training=False,
                                        bank_embedding=bank_embedding_model_input_test,
                                    )
                            ...
                            for name, loss in m.items():
                                if self.rank == 0:
                                    self.logger.add_scalar(
                                        f"loss_val/{name}", loss, self.total_iter
                                    )
                        torch.cuda.empty_cache()
